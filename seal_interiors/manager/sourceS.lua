	...
	if resultRows then
		...
		if resultRow then
			...
			if isElement(sourcePlayer) then
				outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen létrehoztál egy interiort. #4adfbf(" .. resultRow.interiorId .. ")", sourcePlayer, 255, 255, 255, true)
				
				exports.seal_logs:logCommand(sourcePlayer, "createInterior", {
...
addEventHandler("createInterior", getRootElement(),
	function (interiorDatas)
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			...
			if interiorId then
				if availableInteriors[interiorId] then
					if not deletedInteriors[interiorId] then
						...
						outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior sikeresen törölve. #4adfbf(" .. interiorId .. ")", source, 255, 255, 255, true)
							
						exports.seal_logs:logCommand(source, "deleteInterior", {interiorId})
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffA kiválasztott interior már törölve van.", source, 255, 255, 255, true)
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffAz interior visszaállításához használd a #d75959/resetinterior#ffffff parancsot.", source, 255, 255, 255, true)
					end
				elseif deletedInteriors[interiorId] then
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffA kiválasztott interior már törölve van.", source, 255, 255, 255, true)
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffAz interior visszaállításához használd a #4adfbf'/resetinterior' #ffffffparancsot.", source, 255, 255, 255, true)
				end
			end
		end
	end
...
function resetInteriorCallback(queryHandle, sourcePlayer)
	...
	if resultRows then
		...
		if resultRow then
			...
			if isElement(sourcePlayer) then
				outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior sikeresen visszaállítva. #4adfbf(" .. resultRow.interiorId .. ")", sourcePlayer, 255, 255, 255, true)
				
				exports.seal_logs:logCommand(sourcePlayer, "resetInterior", {resultRow.interiorId})
			end
		end
	end
end
	...
	function (sourcePlayer, commandName, interiorId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				outputChatBox("#4adfbf[Használat]: #FFFFFF/" .. commandName .. " [Interior ID]", sourcePlayer, 255, 255, 255, true)
			else
				...
				elseif availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior sikeresen visszaállítva. #4adfbf(" .. interiorId .. ")", sourcePlayer, 255, 255, 255, true)
						
					exports.seal_logs:logCommand(sourcePlayer, "resetInterior", {interiorId})
				else
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
		...
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newName then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior sikeresen átnevezve. #4adfbf(" .. newName .. ")", source, 255, 255, 255, true)
						
					exports.seal_logs:logCommand(source, eventName, {interiorId, newName})
				end
			end
		end
	end
		...
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newPrice then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior ára sikeresen módosítva. #4adfbf(" .. formatNumber(newPrice) .. "$)", source, 255, 255, 255, true)
						
					exports.seal_logs:logCommand(source, eventName, {interiorId, newPrice})
				end
			end
		end
	end
		...
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newType then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior típusa sikeresen módosítva.", source, 255, 255, 255, true)
					
					exports.seal_logs:logCommand(source, eventName, {interiorId, newType})
				end
			end
		end
	end
		...
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior bejárata sikeresen áthelyezve.", source, 255, 255, 255, true)
					
					exports.seal_logs:logCommand(source, eventName, {interiorId})
				end
			end
		end
	end
...
addEventHandler("setInteriorExit", resourceRoot,
	function (interiorId, newDatas)
		if isElement(source) and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					newDatas["dummy"] = "N"
					...
					if source == client then
						outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior kijárata sikeresen áthelyezve.", source, 255, 255, 255, true)
		...
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior belső kinézete sikeresen lecserélve.", source, 255, 255, 255, true)
						
					exports.seal_logs:logCommand(source, eventName, {interiorId, newDatas["gameInterior"]})
				end
			end
		end
	end
		...
		if isElement(source) and client == source and getElementData(client, "acc.adminLevel") >= 6 then
			if interiorId and newDatas then
				if availableInteriors[interiorId] then
					...
					outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior szerkeszthetővé alakítva. (" .. newDatas["editable"] .. ")", source, 255, 255, 255, true)
						
					exports.seal_logs:logCommand(source, eventName, {interiorId, newDatas["editable"]})
				end
			end
		end
	end
...
addCommandHandler("setinteriorowner",
	function (sourcePlayer, commandName, interiorId, partialNick)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
			...
			if not interiorId then
				outputChatBox("#4adfbf[Használat]: #FFFFFF/" .. commandName .. " [Interior ID] [Játékos Név / ID]", sourcePlayer, 255, 255, 255, true)
			else
					...
					if int.type ~= "building" and int.type ~= "rentable" then
						...
						if isElement(targetPlayer) then
							...
							if characterId then
								...
								if exports.seal_items:hasSpaceForItem(targetPlayer, 2) then
									if not exports.seal_items:hasItemWithData(targetPlayer, 2, interiorId) then
										exports.seal_items:giveItem(targetPlayer, 2, 1, false, interiorId)
									end
								end
								...
								exports.seal_gui:showInfobox(sourcePlayer, "s", "Sikeresen megváltoztattad a kiválasztott interior tulajdonosát!")
							end
						end
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffKözépületnek és bérlakásnak nem lehet tulajdonost beállítani.", sourcePlayer, 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
...
function changeInteriorOwnerSell(interiorId, characterId)
	--if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
		interiorId = tonumber(interiorId)
...
function changeInteriorOwner(interiorId, characterId)
	if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
		interiorId = tonumber(interiorId)
...
function getInteriorOwnerCallback(queryHandle, sourcePlayer)
	if isElement(sourcePlayer) then
		...
		if resultRows then
			...
			if resultRow then
				outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffA kiválasztott interior tulajdonosa #4adfbf" .. resultRow.name .. " [Karakter ID: " .. resultRow.characterId .. " - Account ID: " .. resultRow.accountId .. "].", sourcePlayer, 255, 255, 255, true)
			end
		end
	end
end
			...
			if not interiorId then
				...
				if int then
					if int.ownerId > 0 then
						...
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffA kiválasztott interiornak nincs tulajdonosa.", sourcePlayer, 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffA kiválasztott interior nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
...
addCommandHandler("getpropertyowner",
	function (sourcePlayer, commandName, interiorId)
	...
	function (posX, posY, posZ, rotX, rotY, rotZ, interior, dimension)
		if isElement(source) and client == source then
			if posX and posY and posZ and rotX and rotY and rotZ and interior and dimension then
				...
				outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen elteleportáltál a kiválasztott interiorhoz.", source, 255, 255, 255, true)

				exports.seal_logs:logCommand(source, commandName, {interiorId})
			end
		end
	end
)

addCommandHandler("parse",
	function(player)
		if getElementData(player, "acc.adminLevel") >= 9 then
			for k, v in pairs(availableInteriors) do
				--[[if v.type == "house" or v.type == "garage" then
					iprint(k .. "ownerid reset")
					dbExec(connection, "UPDATE interiors SET ownerId = ? WHERE interiorId = ?", 0, k)
				end
				if v.type == "business_passive" or v.type == "business_active" then
					iprint(k .. "ownerid delete")
					dbExec(connection, "UPDATE interiors SET deleted = 'Y' WHERE interiorId = ?", k)
				end
				if v.editable == "Y" then
					iprint(k .. "editable delete")
					dbExec(connection, "UPDATE interiors SET deleted = 'Y' WHERE interiorId = ?", k)
				end
				if tonumber(v.entrance.position[1]) < 85.7654571533 then
					iprint(k .. "kint van a mapbol delete")
					dbExec(connection, "UPDATE interiors SET deleted = 'Y' WHERE interiorId = ?", k)
				end]]
			end
		end
