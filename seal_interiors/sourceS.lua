					...
					for i = 1, #playersTable do
						...
						if isElement(playerElement) then
							...
							if customInterior > 0 and customInterior == interiorId then
								...
								exports.seal_hud:showInfobox(playerElement, "i", "Az interior szerkesztés alatt áll.\nEzért automatikusan kikerültél az ingatlanból.")
							end
						end
					end

					exports.seal_interioredit:loadInterior(source, interiorId, true, true)
				end
			end
		end
	end
			...
			if interiorId then
				...
				if int then
					if int.locked == "N" then
						...
						for i = 1, #playersTable do
							...
							if isElement(playerElement) then
								...
								if editingInterior > 0 and editingInterior == interiorId then
									outputChatBox("#d75959[SealMTA - Interior]: #ffffffAz ingatlan szerkesztés alatt áll.", source, 255, 255, 255, true)
										...
										if getElementType(sourceElement) == "player" then
											...
											if colShapeType == "enter" then
												if warpData.editable ~= "N" then
													...
													exports.seal_interioredit:loadInterior(sourceElement, interiorId)
												end
											end
						...
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffAz ingatlan ajtaja #d75959be van zárva.", source, 255, 255, 255, true)
						triggerClientEvent(source, "playInteriorSound", source, "locked")
					end
				end
			end
		...
		if isElement(source) and client == source then
			if interiorId then
				...
				if int then
					...
					if characterId then
						if exports.seal_items:hasSpaceForItem(source, 2) then
							...
							if exports.seal_core:takeMoney(source, buyPrice) then
								local renewalTime = 0
								...
								if not exports.seal_items:hasItemWithData(source, 2, interiorId) then
									exports.seal_items:giveItem(source, 2, 1, false, interiorId)
								end

								if int.type == "rentable" then
									exports.seal_accounts:showInfo(source, "s", "Sikeresen kibérelted a kiválasztott ingatlant.")
									outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen kibérelted a kiválasztott ingatlant.", source, 255, 255, 255, true)
								else
									exports.seal_accounts:showInfo(source, "s", "Sikeresen megvásároltad a kiválasztott ingatlant.")
									outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen megvásároltad a kiválasztott ingatlant.", source, 255, 255, 255, true)
								end

								dbExec(connection, "UPDATE interiors SET ownerId = ?, renewalTime = ? WHERE interiorId = ?", characterId, renewalTime, interiorId)
							else
								if int.type == "rentable" then
									outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs elég pénzed, hogy kifizesd a bérleti díjat és kauciót.", source, 255, 255, 255, true)
								else
									outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs elég pénzed az ingatlan megvásárlásához. (#4adfbf" .. formatNumber(int.price) .. " $#ffffff)", source, 255, 255, 255, true)
								end
							end
						else
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffNem fér el az inventorydban a kulcs!", source, 255, 255, 255, true)
						end
					end
				end
			end
						...
						if characterId then
							if int.ownerId == characterId then
								...
								if int.renewalTime - dayTimeDuration < currentTimestamp then
									if exports.seal_core:takeMoneyEx(source, int.price) then
										local renewalTime = currentTimestamp + rentTimeDuration

										int.renewalTime = renewalTime

										exports.seal_accounts:showInfo(source, "s", "Sikeresen meghosszabítottad a bérleted 1 héttel!")
										outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen meghosszabítottad a bérleted 1 héttel!", source, 255, 255, 255, true)

										dbExec(connection, "UPDATE interiors SET renewalTime = ? WHERE interiorId = ?", renewalTime, interiorId)
									else
										outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs elég pénzed, hogy kifizesd a bérleti díjat.", source, 255, 255, 255, true)
									end
								else
									outputChatBox("#d75959[SealMTA - Interior]: #ffffffMég nem tudod meghosszabítani a bérleted. (Maximum 1 nappal a lejárta előtt lehet)", source, 255, 255, 255, true)
								end
							end
						end
					end
						...
						if characterId then
							if characterId == int.ownerId then
								if unownInterior(interiorId) then
									...
									exports.seal_core:giveMoney(source, int.price * 4)

									exports.seal_accounts:showInfo(source, "s", "Sikeresen felmondtad az ingatlan bérlését!")
									outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen felmondtad az ingatlan bérlését!", source, 255, 255, 255, true)
		...
		if int then
			...
			for i = 1, #playersTable do
				...
				if isElement(playerElement) then
					...
					if characterId then
						if int.ownerId == characterId then
							local itemsTable = exports.seal_items:getElementItems(playerElement) or {}

							for k, v in pairs(itemsTable) do
								if v.itemId == 2 and tonumber(v.data1) == interiorId then
									exports.seal_items:takeItem(playerElement, "dbID", v.dbID)
									break
								end
							end
	...
	for k, v in pairs(availableInteriors) do
		if v.ownerId > 0 and v.renewalTime > 0 then
			...
			if isElement(ownerPlayer) then
				if currentTimestamp >= v.renewalTime then
					exports.seal_accounts:showInfo(ownerPlayer, "e", "Nem fizetted ki az albérleted, ezért lejárt!")
					outputChatBox("#d75959[SealMTA - Interior]:#ffffff Nem fizetted ki az albérleted, ezért lejárt!", ownerPlayer, 255, 255, 255, true)
				else
					if v.renewalTime - dayTimeDuration <= currentTimestamp then
						...
						if timeLeft then
							exports.seal_accounts:showInfo(ownerPlayer, "e", "Hamarosan lejár az albérleted, részletek a chatboxban!")
							outputChatBox("#d75959[SealMTA - Interior]:#ffffff Hamarosan lejár az albérleted! (Még #d75959kb. " .. timeLeft .. " óra#ffffff)", ownerPlayer, 255, 255, 255, true)
							outputChatBox("#d75959[SealMTA - Interior]:#ffffff Menj az ingatlanhoz, és írd be a #4adfbf/rent#ffffff parancsot, hogy meghosszabítsd, vagy mondd le az albérletet a #d75959/unrent#ffffff paranccsal. (InteriorID: #598ed7" .. k .. "#ffffff)", ownerPlayer, 255, 255, 255, true)
						end
					end
				end
			end
		...
		if isElement(source) and client == source then
			if interiorId then
				...
				if int then
					...
					local hasKey = exports.seal_items:hasItemWithData(source, 2, interiorId)
					...
					if adminDuty == 1 or hasKey then
						...
						if isLocked == "N" then
							outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffSikeresen #4adfbfkinyitottad #ffffffaz ingatlan ajtaját.", source, 255, 255, 255, true)
						else
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffSikeresen #d75959bezártad #ffffffaz ingatlan ajtaját.", source, 255, 255, 255, true)
						end
						...
						if eventName then
							exports.seal_logs:logCommand(source, eventName, {
								"id: " .. interiorId,
								"locked: " .. isLocked,
						...
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs kulcsod ehhez az ingatlanhoz.", source, 255, 255, 255, true)
					end
				end
			end
		end
...
addEventHandler("useLawlock", getRootElement(), function(interiorId, reason)
	if isElement(client) then
		if interiorId then
			if availableInteriors[interiorId] then
				triggerClientEvent("useLawlock", source, interiorId, reason, getElementData(client, "char.Name"))
				dbExec(connection, "UPDATE interiors SET lawlock = ?, lawlockby = ? WHERE interiorId = ?", reason, getElementData(client, "char.Name"), interiorId)
			end
		end
	end
end)