...
local coolMarkerResources = {}

addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		selectionTex = dxCreateTexture("files/icons/selection.png")
...
addEventHandler("onClientResourceStop", root,
	function (resourceElement)
		local validInterior = coolMarkerResources[resourceElement]

		if validInterior then
			for markerId, markerData in pairs(validInterior) do
				local markerElement = markerData[1]
				local colElement = markerData[2]

				if markerElement then
					if coolMarkersKeyed[markerElement] then
						coolMarkersKeyed[markerElement] = nil
					end

					if coolMarkersInteriorId[markerElement] then
						coolMarkersInteriorId[markerElement] = nil
					end
				end

				if markerId then
					if coolMarkers[markerId] then
				...
				if isElement(colElement) then
					destroyElement(colElement)
				end

				if coolMarkerResources[resourceElement] and coolMarkerResources[resourceElement][markerId] then
					coolMarkerResources[resourceElement] = nil
				end
			end
		end
	end
...
function createInterior(interiorId)
	...
	if int then
		...
		int.enterPickup, int.enterPickupCol = createCoolMarker(int.entrance.position[1], int.entrance.position[2], int.entrance.position[3], int.type, state, int.interiorId)
		...
		if int.enterPickup then
			...
			setElementInterior(int.enterPickupCol, int.entrance.interior)
			setElementDimension(int.enterPickupCol, int.entrance.dimension)
			setElementData(int.enterPickupCol, "interiorId", interiorId, false)
			setElementData(int.enterPickupCol, "colShapeType", "entrance", false)
		end
		
		int.exitPickup, int.exitPickupCol = createCoolMarker(int.exit.position[1], int.exit.position[2], int.exit.position[3], int.type, state)
		...
		if int.exitPickup then
			...
			setElementInterior(int.exitPickupCol, int.exit.interior)
			setElementDimension(int.exitPickupCol, int.exit.dimension)
			setElementData(int.exitPickupCol, "interiorId", interiorId, false)
			setElementData(int.exitPickupCol, "colShapeType", "exit", false)
		end
	end
end
		...
		if hitElement == localPlayer and dimensionMatch then
			...
			if interiorId and colShapeType then
				...
				if int.ownerId == 0 then
					if int.type ~= "building" and int.type ~= "building2" then
						if int.type == "rentable" then
							outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffEz az ingatlan kiadó! A bérléshez használd a #4adfbf/rent #ffffffparancsot!", 255, 255, 255, true)
							outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffBérleti díj: #4adfbf" .. formatNumber(int.price) .. " $/hét.#ffffff Kaució: #4adfbf" .. formatNumber(int.price * 4) .. " $", 255, 255, 255, true)
						else
							outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffEz az ingatlan eladó! A vásárláshoz használd a #4adfbf/buy #ffffffparancsot! Ár: #4adfbf" .. formatNumber(int.price) .. " $", 255, 255, 255, true)
						end
					...
					if int.ownerId == tonumber(getElementData(localPlayer, "char.ID")) then
						if int.editable ~= "N" then
							if colShapeType == "entrance" then
								outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffEz az ingatlan szerkeszthető!", 255, 255, 255, true)
								outputChatBox("#4adfbf[SealMTA - Interior]: #ffffffHasználd a #4adfbf[Z]#ffffff gombot vagy a '#4adfbf/edit#ffffff' parancsot a szerkesztéshez!", 255, 255, 255, true)
								exports.seal_hud:showInfobox("i", "Ez egy szerkeszthető interior! Részletek a chatboxban.")
							end
						end
					end
				end
				...
				if colShapeType == "entrance" then
					...
					exports.seal_hud:showInteriorBox("[" .. interiorId .. "] " .. int.name, "Nyomj [E] gombot a belépéshez.", int.locked, int.type, showPreview and int.gameInterior or false)
				elseif colShapeType == "exit" then
					exports.seal_hud:showInteriorBox("[" .. interiorId .. "] " .. int.name, "Nyomj [E] gombot a kilépéshez.", int.locked, int.type)
				end
			end
		end
	end
	...
	function ()
		...
		if interiorInfo then
			...
			if interiorData then
				...
				dxDrawText("[" .. interiorInfo["interiorId"] .. "]", x + respc(10), math.floor(y + (sy - respc(64) - fontHeight) / 2) + respc(64), x + respc(74), 0, tocolor(r, g, b, 255 * progress), 0.75, Rubik, "center", "top")
				
				-- ** Információk
				dxDrawText(interiorData["name"], x + respc(58), y, x + interiorInfo["panelSizeX"], y + sy - fontHeight, tocolor(255, 255, 255, 255 * progress), 0.85, Rubik, "center", "center", true)
				dxDrawText(interiorInfo["infoText"], x + respc(58), y + fontHeight, x + interiorInfo["panelSizeX"], y + sy, tocolor(255, 255, 255, 255 * progress), 0.8, RubikL, "center", "center", true)
				...
				if int.type == "rentable" then
					...
					if int.ownerId == characterId then
						if getTickCount() >= lastInteriorBuy + 5000 then
							...
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffCsak 5 másodpercenként használhatod ezt a parancsot.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffEzt az ingatlant nem te bérled.", 255, 255, 255, true)
					end
				end
			end
		end
		...
		if standingMarker then
			...
			if int then
				if int.type == "rentable" then
					...
					if int.ownerId == 0 then
						if getElementData(localPlayer, "char.Money") >= int.price * 5 then
							...
							if interiorCount < interiorLimit then
								...
								outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs szabad ingatlan slotod.", 255, 255, 255, true)
							end
						else
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs elég pénzed, hogy kifizesd a bérleti díjat és kauciót.", 255, 255, 255, true)
						end
					elseif int.ownerId == characterId then
						if exports.seal_core:getMoney(localPlayer) >= int.price then
							if getTickCount() >= lastInteriorBuy + 5000 then
							...
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs elég pénzed, hogy kifizesd a bérleti díjat.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffEz a bérlakás nem kiadó.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffEzt az ingatlant nem lehet kibérelni.", 255, 255, 255, true)
				end
			end
		end
	end)
...
addCommandHandler("buy",
	function ()
		if standingMarker then
			...
			if int then
				if int.type == "rentable" then
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffBérelhető lakást nem vásárolhatsz meg.", 255, 255, 255, true)
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffBérléshez használd a #598ed7/rent#ffffff parancsot.", 255, 255, 255, true)
					return
				end

				if int.type ~= "building" and int.type ~= "building2" then
					if int.ownerId == 0 then
						if getElementData(localPlayer, "char.Money") >= int.price then
							local interiorLimit = getElementData(localPlayer, "player.interiorLimit") or 5
							local interiorCount = 0
							...
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffNincs elég pénzed az ingatlan megvásárlásához. (#4adfbf" .. formatNumber(int.price) .. " $#ffffff)", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffEz az ingatlan nem eladó.", 255, 255, 255, true)
					end
				else
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffKözépületet nem vásárolhatsz meg.", 255, 255, 255, true)
				end
			end
		end
	end)
			...
			if int and int.type ~= "building" then
				...
				if characterId then
					if getTickCount() >= lastInteriorLock + 5000 then
						...
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffCsak 5 másodpercenként #4adfbfnyithatod #ffffff/ #d75959zárhatod #ffffffaz ajtót.", 255, 255, 255, true)
					end
				end
			end
		end
	...
	function (interiorId, locked)
		if interiorId then
			...
			if int then
				...
				if standingMarker and standingMarker[1] == interiorId then
					exports.seal_hud:setInteriorDoorState(int.locked, int.type)
				end
			end
		end
	end)
	...
	function ()
		if standingMarker then
			if getElementData(localPlayer, "editingInterior") then
				exports.seal_hud:showInfobox("e", "Nem hagyhatod el az interiort szerkesztő módban.")
				return
			end
					...
					if not int.dummy or int.dummy == "N" then
						if not getElementData(localPlayer, "cuffed") and not getElementData(localPlayer, "tazed") then
							if getTickCount() >= lastInteriorEnter + 3000 then
								...
								if colShapeType == "entrance" and int.lawlock and int.lawlock ~= "0" then
									outputChatBox("#d75959[SealMTA - Interior]: #ffffffEz az interior le van zárva! #d75959(" .. int.lawlock .. ")", 255, 255, 255, true)
									if getElementData(localPlayer, "acc.adminLevel") > 0 then
										outputChatBox("#d75959[SealMTA - Interior]: #ffffffLezárta: #d75959((" .. int.lawlockby .. "))", 255, 255, 255, true)
									end
									return
								end
								...
								outputChatBox("#d75959[SealMTA - Interior]: #ffffffVárj egy kicsit, mielőtt újra használod az ajtót.", 255, 255, 255, true)
							end
						else
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffMegbilincselve/lesokkolva nem használhatod az ajtót.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffEz az interior nem rendelkezik tényleges belső térrel.", 255, 255, 255, true)
					end
				end
			end
		end
...
function useDoorRammer()
	if exports.seal_groups:isPlayerHavePermission(localPlayer, "doorRammer") then
			...
			if interiorId then
				...
				if int then
					if int.type ~= "building" then
						...
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffItt nem használhatod a faltörő kost.", 255, 255, 255, true)
					end
				end
			end
		else
		...
		outputChatBox("#d75959[SealMTA - Interior]: #ffffffNem használhatod a faltörő kost.", 255, 255, 255, true)
	end
end

local lastKnocked, lastBelled = 0, 0
		...
		if interiorId then
			...
			if int then
				if int.type == "house" or int.type == "garage" or int.type == "rentable" then
					if (getTickCount() - lastKnocked) > 5000 then
						lastKnocked = getTickCount()
						triggerServerEvent("useDoorKnocking", localPlayer, interiorId)
					else
						outputChatBox("[SealMTA]: #ffffffVárj 5 másodpercet a következő interakcióig!", 215, 89, 89, true)
					end
				else
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffCsak házba vagy garázsba kopogtathatsz.", 255, 255, 255, true)
				end
			end
		end
	else
		outputChatBox("#d75959[SealMTA - Interior]: #ffffffItt nem kopogtathatsz.", 255, 255, 255, true)
	end
end
	...
	if standingMarker then
		...
		if interiorId then
			...
			if int then
				if int.type == "house" or int.type == "garage" or int.type == "rentable" then
					if getElementDimension(localPlayer) ~= 0 then
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffCsak kintről csengethetsz be.", 255, 255, 255, true)
					else
						if (getTickCount() - lastBelled) > 5000 then
							lastBelled = getTickCount()
					...
					outputChatBox("#d75959[SealMTA - Interior]: #ffffffCsak házba vagy garázsba csengethetsz.", 255, 255, 255, true)
				end
			end
		end
	else
		outputChatBox("#d75959[SealMTA - Interior]: #ffffffItt nem csengethetsz.", 255, 255, 255, true)
	end
end
		...
		if interiorId then
			...
			if int then
				...
				if source == localPlayer then
					exports.seal_chat:localActionC(localPlayer, "betöri az ajtót.")
				end
			end
		end
	end)
...
iconPictures.tuning = dxCreateTexture("files/icons/tuning.png")
iconPictures.barn = dxCreateTexture("files/icons/barn.png")
iconPictures.barn2 = dxCreateTexture("files/icons/barn2.png")
iconPictures.dontcross = dxCreateTexture("files/icons/donotcross.png")

local iconColors = {}

iconColors.house = {89, 142, 215}
iconColors.houseforsale = {94, 193, 230}
iconColors.info = {220, 163, 0}
iconColors.garage = {89, 142, 215}
iconColors.garageforsale = {94, 193, 230}
...
iconColors.tuning = {200, 210, 245}
iconColors.barn = {94, 193, 230}
...
function createCoolMarker(posX, posY, posZ, markerType, state, interiorId, resourceElement, skipInit)
	markerType = getInteriorMarkerType(markerType, state)
	
	local markerId = #coolMarkers + 1
	local markerElement = createMarker(posX, posY, posZ - 1, "cylinder", 0, iconColors[markerType][1], iconColors[markerType][2], iconColors[markerType][3], 125)
	local colShapeElement = false

	if not skipInit then
		print("asd")
		colShapeElement = createColSphere(posX, posY, posZ, 0.8)
	end
	...
	if interiorId then
		coolMarkersInteriorId[markerElement] = interiorId
	end

	if resourceElement then
		if not coolMarkerResources[resourceElement] then
			coolMarkerResources[resourceElement] = {}
		end

		coolMarkerResources[resourceElement][markerId] = {markerElement, colShapeElement}
	end
	
	return markerElement, colShapeElement, markerId
end

function destroyCoolMarker(markerElement, colShapeElement, markerId)
	if markerId and markerElement and colShapeElement then
		if coolMarkers[markerId] then
			coolMarkers[markerId] = nil
		end

		if coolMarkersKeyed[markerElement] then
			coolMarkerKeyed[markerElement] = nil
		end
		...
		for k = 1, #nearbyMarkers do
			...
			if isElement(v[3]) then
				if getElementInterior(v[3]) == cameraInterior then
					...
					dxDrawMaterialLine3D(markerPosX, markerPosY, markerPosZ + 0.2 + size + 0.2, markerPosX, markerPosY, markerPosZ - 0.2 + size + 0.2, iconPictures[typ], 0.4, tocolor(iconColors[typ][1], iconColors[typ][2], iconColors[typ][3]))

					local cx, cy, cz = getCameraMatrix()
					if availableInteriors[coolMarkersInteriorId[v[3]]] and availableInteriors[coolMarkersInteriorId[v[3]]].lawlock and availableInteriors[coolMarkersInteriorId[v[3]]].lawlock ~= "0" then	
						local markerPosX = markerPosX - (markerPosX-cx)/100
						local markerPosY = markerPosY - (markerPosY-cy)/100
						local markerPosZ = markerPosZ - (markerPosZ-cz)/100
						dxDrawMaterialLine3D(markerPosX, markerPosY, markerPosZ + 0.3 + size + 0.2, markerPosX, markerPosY, markerPosZ - 0.3 + size + 0.2, iconPictures.dontcross, 0.46)
					end
					dxDrawMaterialLine3D(markerPosX, markerPosY, markerPosZ + 0.26 + size - 0.3, markerPosX, markerPosY, markerPosZ - 0.26 + size - 0.3, selectionTex, 0.64, tocolor(iconColors[typ][1], iconColors[typ][2], iconColors[typ][3]))
				end
			end
		end
	end, true, "low-999"
				...
				if int then
					if int.editable ~= "N" then
						...
						if int.ownerId == characterId then
							...
							outputChatBox("#d75959[SealMTA - Interior]: #ffffffEz az ingatlan nem a te tulajdonod.", 255, 255, 255, true)
						end
					else
						outputChatBox("#d75959[SealMTA - Interior]: #ffffffEz nem szerkeszthető interior.", 255, 255, 255, true)
					end
				end
			end
		end
...
addCommandHandler("lawlock", function(commandName, ...)
	reason = table.concat({...}, " ")
	if exports.seal_groups:isPlayerOfficer(localPlayer) and reason and utfLen(reason) > 0 then
		if standingMarker then
			local interiorId = tonumber(standingMarker[1])
			local colShapeType = tostring(standingMarker[2])

			if interiorId and colShapeType == "entrance" then
				triggerServerEvent("useLawlock", localPlayer, interiorId, reason)
			end
		else
			outputChatBox("#d75959[SealMTA - Interior]: #ffffffNem állsz interior CP-ben!", 255, 255, 255, true)
		end
	else
		outputChatBox("#d75959[SealMTA - Interior]: #ffffff/lawlock [Indok (0 = levétel)]", 255, 255, 255, true)
	end
end)

addEvent("useLawlock", true)
addEventHandler("useLawlock", getRootElement(), function(interiorId, reason, lockedBy)
	if interiorId then
		if availableInteriors[interiorId] then
			availableInteriors[interiorId].lawlock = reason
			availableInteriors[interiorId].lawlockby = lockedBy
		end
	end
end)