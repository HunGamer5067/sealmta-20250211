...
local availableMarkerPoints = {
	...
	{2435.3891601562, -2719.3107910156, -0.55000001192093}
}
					...
					if currentVehicle then
						...
						if vehicleModel == 453 or vehicleModel == 454 then
							...
							if numOfCrates > 0 then
								exports.seal_hud:showInfobox("e", "Előbb add le a másik fajta ládákat.")
								return
							end
					...
					if lastCrateIndex > 0 then
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, crateX, crateY, crateZ) <= 2.5 then
							...
							if guiPosX and guiPosY then
								...
								dxDrawImage(primaryPosX, buttonPosY, buttonSize, buttonSize, "files/cage_down.png", 0, 0, 0, tocolor(74, 223, 191, cageDropAlpha))
								...
								dxDrawImage(secondaryPosX, buttonPosY, buttonSize, buttonSize, "files/cage_fish.png", 0, 0, 0, tocolor(74, 223, 191, cageBaitAlpha))
							end
						end
					end
							...
							if not isElement(objectElement) then
								...
								if getDistanceBetweenPoints3D(playerX, playerY, playerZ, crateX, crateY, crateZ) <= maximumDistance then
									...
									if guiPosX and guiPosY then
										...
										dxDrawImage(guiPosX, guiPosY, guiSize, guiSize, "files/cage_up.png", 0, 0, 0, tocolor(74, 223, 191, cageUpAlpha))
									end
								end
							end
						end
					...
					if contactModel == 454 then
						...
						if rightDoorDistance <= 0.65 or leftDoorDistance <= 0.65 then
							...
							if not isVehicleLocked(contactElement) then
								dxDrawText("Belépéshez: #bc873d/enter", 0, screenY - respc(128), screenX, screenY - respc(88), tocolor(255, 255, 255), 1, Roboto, "center", "center", false, false, false, true)
							else
				...
				if getElementInterior(localPlayer) == 123 then
					...
					if rightDoorDistance <= 0.65 or leftDoorDistance <= 0.65 then
						...
						if not isLocked then
							dxDrawText("Kilépéshez: #bc873d/enter", 0, screenY - respc(128), screenX, screenY - respc(88), tocolor(255, 255, 255), 1, Roboto, "center", "center", false, false, false, true)
						else
							dxDrawText("Az ajtó #d75959zárva #ffffffvan.", 0, screenY - respc(128), screenX, screenY - respc(88), tocolor(255, 255, 255), 1, Roboto, "center", "center", false, false, false, true)
						end
					end
		...
		if button == "left" then
			if state == "up" then
				if activeCageButton then
					print(activeCageButton)
					if getTickCount() - lastClickTick <= 1000 then
						exports.seal_hud:showInfobox("e", "Ilyen gyorsan nem kéne!")
						return
					end
...
function renderCrabMenu()
	...
	if myShipCrabWeight > 0 then
		dxDrawText("Hajó tartalma: #598ed7" .. math.floor(myShipCrabWeight * 10) / 10 .. " kg#ffffff rák.\nEgységár: #bc873d" .. math.floor(crabPrice * 10) / 10 .. " $/kg\n#ffffffÉrték: #bc873d" .. math.floor(crabPrice * myShipCrabWeight + myShipCagesPrice) .. " $", menuPosX, menuPosY + respc(10), menuPosX + menuWidth, 0, tocolor(255, 255, 255), 0.8, Roboto, "center", "top", false, false, false, true)
	else
		dxDrawText("Ketrecek:\n(#bc873d" .. cagePrice .. "$#ffffff/ketrec)", menuPosX, menuPosY + respc(10), menuPosX + menuWidth, 0, tocolor(255, 255, 255), 0.8, Roboto, "center", "top", false, false, false, true)
	...
	if myShipCrabWeight > 0 then
		...
		dxDrawRectangle(buttonPosX, menuPosY + menuHeight - respc(42), respc(120), respc(32), tocolor(74, 223, 191, buttonAlpha))
		...
		if myShipCagesCount > 0 then
			...
			dxDrawRectangle(menuPosX + respc(40), menuPosY + menuHeight - respc(42), respc(32), respc(32), tocolor(74, 223, 191, buttonAlpha))
		...
		if myShipCagesCount < maximumSlot then
			...
			dxDrawRectangle(menuPosX + menuWidth - respc(72), menuPosY + menuHeight - respc(42), respc(32), respc(32), tocolor(74, 223, 191, buttonAlpha))
		...
		if shipId == 0 then
			...
			if isElement(shipElement) then
				if isVehicleLocked(shipElement) then
					outputChatBox("#d75959[WeakMTA]: #ffffffBe van zárva a hajó.", 255, 255, 255, true)
					return
				end
