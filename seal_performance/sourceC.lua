...
addCommandHandler("performance",
	function (commandName, targetPlayer)
		if getElementData(localPlayer, "acc.adminLevel") >= 7 and not targetPlayer then
			...
			if panelState then
				RubikFont = dxCreateFont("Rubik.ttf", respc(12), false, "proof")
	...
	for i = 1, #statsColumn do
		...
		for j = 1, #statsRow do
			...
			if performanceCategory == "Lua timing" and (i == 2 or i == 7 or i == 12) then
				local rowColor = tocolor(94, 193, 230, 125)
				...
				if statsRow[j] and statsRow[j][i] then
					...
					if number and tonumber(number) then
						...
						local r, g, b = 94, 193, 230

						if progress > 0.5 then
							r, g, b = interpolateBetween(220, 163, 0, 215, 89, 89, progress, "Linear")
						else
							r, g, b = interpolateBetween(94, 193, 230, 220, 163, 0, progress, "Linear")
						end
			...
			dxDrawText(statsRow[j][i], x_offset + 1, y, x_offset + 1 + rowWidth - 1, y + rowHeight, tocolor(255, 255, 255), 1.0, RubikFont, "center", "center", true)
		end
...
function renderPerformance()
	...
	local titleWidth = dxGetTextWidth("Insight#4adfbfMTA", 1, RubikFont, true) + 5

	dxDrawText("Insight#4adfbfMTA", x + 5, y2, 0, y, tocolor(255, 255, 255), 1, RubikFont, "left", "center", false, false, false, true)
	dxDrawText("Performance Browser - ", x + titleWidth + 10, y2, x, y, tocolor(255, 255, 255), 1, RubikFont, "left", "center")

	titleWidth = titleWidth + dxGetTextWidth("Performance Browser - ", 1, RubikFont)

	dxDrawText("SampleRate: " .. sampleRate, x + titleWidth + 10, y2, x, y, tocolor(255, 255, 255), 0.8, RubikFont, "left", "center")
	
	titleWidth = titleWidth + dxGetTextWidth("SampleRate: " .. sampleRate, 0.8, RubikFont) + 5

	if cx >= x + titleWidth + 10 and cy >= y - buttonSize2 and cx <= x + titleWidth + 10 + buttonSize and cy <= y - buttonSize2 + buttonSize then
		dxDrawRectangle(x + titleWidth + 10, y - buttonSize2, buttonSize, buttonSize, tocolor(94, 193, 230, 175))
		activeButton = "sampleRate:plus"
	else
		dxDrawRectangle(x + titleWidth + 10, y - buttonSize2, buttonSize, buttonSize, tocolor(50, 50, 50, 200))
	end
	dxDrawText("+", x + titleWidth + 10, y - buttonSize2, x + titleWidth + 10 + buttonSize, y - buttonSize2 + buttonSize, tocolor(255, 255, 255), 1, RubikFont, "center", "center")
	...
	dxDrawText("Type: " .. performanceElementType .. "-" .. performanceCategory, x + titleWidth + 10, y2, x, y, tocolor(255, 255, 255), 0.8, RubikFont, "left", "center")

	titleWidth = titleWidth + dxGetTextWidth("Type: " .. performanceElementType .. "-" .. performanceCategory, 0.8, RubikFont) + 10
	
	local performanceSide = performanceElementType == "client" and "server" or "client"
	local sideWidth = dxGetTextWidth("Switch to " .. performanceSide .. " side", 1, RubikFont) + 10

	if cx >= x + titleWidth + 10 and cy >= y - buttonSize2 and cx <= x + titleWidth + 10 + sideWidth and cy <= y - buttonSize2 + buttonSize then
		dxDrawRectangle(x + titleWidth + 10, y - buttonSize2, sideWidth, buttonSize, tocolor(94, 193, 230, 175))
		activeButton = "side:" .. performanceSide
	else
		dxDrawRectangle(x + titleWidth + 10, y - buttonSize2, sideWidth, buttonSize, tocolor(50, 50, 50, 200))
	end
	dxDrawText("Switch to " .. performanceSide .. " side", x + titleWidth + 10, y - buttonSize2, x + titleWidth + 10 + sideWidth, y - buttonSize2 + buttonSize, tocolor(255, 255, 255), 1, RubikFont, "center", "center")

	titleWidth = titleWidth + sideWidth + 5
	sideWidth = dxGetTextWidth(performanceCategories[performanceCategoryId + 1] or performanceCategories[1], 1, RubikFont) + 10
	...
	dxDrawText(performanceCategories[performanceCategoryId + 1] or performanceCategories[1], x + titleWidth + 10, y - buttonSize2, x + titleWidth + 10 + sideWidth, y - buttonSize2 + buttonSize, tocolor(255, 255, 255), 1, RubikFont, "center", "center")

	if performanceElementType == "anotherClient" and isElement(performanceElement) then
		dxDrawText("Client: " .. string.gsub(string.gsub(getPlayerName(performanceElement), "_", " "), "#%x%x%x%x%x%x", ""), x, y2, x + panelWidth - 10, y, tocolor(255, 255, 255), 0.8, RubikFont, "right", "center")
	end
	...
	for i = 1, #statsColumn do
		...
		if cx >= x_offset and cy >= y and cx <= x_offset + rowWidth and cy <= y + rowHeight and not activeButton then
			...
			dxDrawRectangle(x_offset, y, rowWidth, rowHeight, tocolor(94, 193, 230, 175))
		end

		if i ~= 1 then
			dxDrawRectangle(x_offset, y, 1, rowHeight, tocolor(0, 0, 0, 255))
		end
		dxDrawText(statsColumn[i], x_offset + 1, y, x_offset + 1 + rowWidth - 1, y + rowHeight, tocolor(255, 255, 255), 1.0, RubikFont, "center", "center", true)
	...
	if isElement(renderTarget) then
		...
		if #statsRow > visibleItem then
			...
			dxDrawRectangle(x + panelWidth - 5, y + (panelHeight / #statsRow) * scrollbarPosition, 5, (panelHeight / #statsRow) * visibleItem, tocolor(94, 193, 230, 200))
		end
	end
end
