...
local unImpoundPed = createPed(281, 1813.3620605469, -2077.5939941406, 13.546875, 270)
setElementData(unImpoundPed, "invulnerable", true)
setElementData(unImpoundPed, "visibleName", "Kjetil Tufte")
setElementFrozen(unImpoundPed, true)
...
addEventHandler("onClientResourceStart", getResourceRootElement(),
	function ()
		havePermission.impoundTow = exports.seal_groups:isPlayerHavePermission(localPlayer, "impoundTow")
		havePermission.impoundTowFinal = exports.seal_groups:isPlayerHavePermission(localPlayer, "impoundTowFinal")
		...
		if dataName == "carAnimation" then
			...
			if dataVal then
				...
				if carAnimation[source][2] == "down" then
					...
					local groundX, groundY, groundPreZ = getPositionFromElementOffset(effect.towedBy, 0, -13, math.abs(minZ))
	...
	function ()
											...
											if tx then
												if i == 4 then
													if holder == localPlayer then
														exports.seal_hud:showInfobox("w", "Itt nem tud átmenni a kábel!")
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				exports.seal_hud:showInfobox("w", "Ilyen messzire nem ér el a kábel!")
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								exports.seal_hud:showInfobox("e", "Előbb rakd a helyére az autószállító rámpáját!")
							end
						end
					end
				end
									...
									if havePermission.impoundTowFinal then
										if isElement(currentHolder) and getElementType(currentHolder) == "vehicle" then
											...
											if isInsideColShape(impoundColshape, tx, ty, tz) then
												triggerServerEvent("impoundTheVehicle", localPlayer, currentHolder, true)
											end
										end
									end
								end
											...
											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												...
												if groupId == 0 then
													...
													if jobVehID == 0 then
														...
														outputChatBox("#d75959[SealMTA - Lefoglalás]: #ffffffMunkajárműveket nem foglalhatsz le.", 255, 255, 255, true)
													end
												else
													outputChatBox("#d75959[SealMTA - Lefoglalás]: #ffffffFrakciójárműveket nem foglalhatsz le.", 255, 255, 255, true)
												end
											else
												outputChatBox("#d75959[SealMTA - Lefoglalás]: #ffffffAz autó nincs az autószállító mögött!", 255, 255, 255, true)
											end
										else
											outputChatBox("#d75959[SealMTA - Lefoglalás]: #ffffffTúl messze vagy a járműtől!", 255, 255, 255, true)
										end
									else
										outputChatBox("#d75959[SealMTA - Lefoglalás]: #ffffffCsak autókat foglalhatsz le.", 255, 255, 255, true)
									end
								end
							end
						end
	...
	function (vehs)
		if vehs and type(vehs) == "table" then
			...
			for k, v in pairs(vehs) do
				impoundedVehicles[k].dbid = v.vehicleId
				impoundedVehicles[k].gtaModelName = getVehicleNameFromModel(v.modelId)
				impoundedVehicles[k].realModelName = exports.seal_vehiclenames:getCustomVehicleName(v.modelId)
				--impoundedVehicles[k].dbId = getElementData(vehs, "vehicle.dbID")
			end
...
function togglePanel(state)
	if state ~= panelState then
		if state then
			panelState = true
			panelFont = dxCreateFont("files/Rubik.ttf", 12, false, "antialiased")
		else
			impoundedVehicles = {}
			panelState = false
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		if panelState then
			...
			dxDrawRectangle(x, y, sx, 30, tocolor(0, 0, 0, 200))
			dxDrawText("Insight#4adfbfMTA #ffffff- Lefoglalt járművek", x + 7.5, y, 0, y + 30, tocolor(255, 255, 255), 1, panelFont, "left", "center", false, false, false, true)

			local tabNum = 13
			local oneSize = (sy - 40) / tabNum

			buttons["close"] = {x + sx - 110, y + 5, 100, oneSize - 10}

			if activeButton == "close" then
				dxDrawRectangle(x + sx - 110, y + 5, 100, oneSize - 10, tocolor(215, 89, 89, 225))
			else
				dxDrawRectangle(x + sx - 110, y + 5, 100, oneSize - 10, tocolor(215, 89, 89, 175))
			end

			dxDrawText("Bezárás", x + sx - 110, y + 5, x + sx - 10, y - 5 + oneSize, tocolor(0, 0, 0), 0.8, panelFont, "center", "center")

			local startY = y + 35

			dxDrawRectangle(x + 5, startY, sx - 10, oneSize, tocolor(0, 0, 0, 225))
			dxDrawText("Jármű neve", x + 10, startY, 0, startY + oneSize, tocolor(94, 193, 230), 0.8, panelFont, "left", "center")
			dxDrawText("Eredeti jármű", x + 260, startY, 0, startY + oneSize, tocolor(94, 193, 230), 0.8, panelFont, "left", "center")
			...
			for i = 1, tabNum do
				...
				if i % 2 ~= 0 then
					dxDrawRectangle(x + 5, y, 490, oneSize, tocolor(0, 0, 0, 150))
				else
					dxDrawRectangle(x + 5, y, 490, oneSize, tocolor(0, 0, 0, 200))
				end

				local veh = impoundedVehicles[i + offsetVeh]

				if veh then
					dxDrawText("(#" .. veh.dbid .. ") " .. veh.realModelName, screenX / 2 - 500 / 2 + 10, screenY / 2 - 400 / 2 + 35 + 360 / 13 * i, 490, screenY / 2 - 400 / 2 + 35 + 360 / 13 * i + 360 / 13, tocolor(255, 255, 255), 0.8, panelFont, "left", "center")
					--dxDrawText(veh.realModelName, x + 10, y, 0, y + oneSize, tocolor(255, 255, 255), 0.8, panelFont, "left", "center")
					dxDrawText(veh.gtaModelName, x + 260, y, 0, y + oneSize, tocolor(255, 255, 255), 0.8, panelFont, "left", "center")

					buttons["unImpound_" .. i + offsetVeh] = {x + sx - 115, y + 5, 100, oneSize - 10}

					if activeButton == "unImpound_" .. i + offsetVeh then
						dxDrawRectangle(x + sx - 115, y + 5, 100, oneSize - 10, tocolor(94, 193, 230, 225))
					else
						dxDrawRectangle(x + sx - 115, y + 5, 100, oneSize - 10, tocolor(94, 193, 230, 175))
					end

					dxDrawText("Kiváltás", x + sx - 115, y + 5, x + sx - 15, y + 5 + oneSize - 10, tocolor(0, 0, 0), 0.8, panelFont, "center", "center")
				end
			end
			...
			if #impoundedVehicles > tabNum then
				...
				dxDrawRectangle(x + sx - 5, startY, 5, trackSize, tocolor(0, 0, 0, 200))
				dxDrawRectangle(x + sx - 5, startY + offsetVeh * (trackSize / #impoundedVehicles), 5, trackSize / #impoundedVehicles * tabNum, tocolor(94, 193, 230, 150))
			end
					...
					if isElement(holder) and getElementType(holder) == "vehicle" then
						if havePermission.impoundTowFinal then
							...
							if sx and sy then
								...
								if isInsideColShape(impoundColshape, tx, ty, tz) then
									dxDrawImage(math.floor(sx - oneSize / 2), math.floor(sy - oneSize / 2), oneSize, oneSize, "files/impound.png", 0, 0, 0, tocolor(94, 193, 230))
								else
						...
						if detected > 0 then
							...
							if sx and sy then
								...
								if isElement(nearbyCar) then
									dxDrawImage(math.floor(sx - oneSize / 2), math.floor(sy - oneSize / 2), oneSize, oneSize, "files/tow.png", 0, 0, 0, tocolor(94, 193, 230))
								else
														...
														if dist < 2 then
															...
															if x and y then
																...
																if absX >= x and absX <= x + oneSize and absY >= y and absY <= y + oneSize then
																	pictureColor = tocolor(94, 193, 230)
