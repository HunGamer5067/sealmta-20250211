		...
		elseif dataName == "loggedIn" then
			if getElementData(localPlayer, "loggedIn") and getElementData(localPlayer, "isPlayerDeath") then
				...
				if getElementData(localPlayer, "cuffed") then
					exports.seal_controls:toggleControl({"jump", "crouch", "walk", "aim_weapon", "fire", "enter_passenger"}, true)
				end
				...
				preDeathTimer = setTimer(
					function ()
						...
						Rubik = dxCreateFont("files/Rubik.ttf", 24, false, "proof")
...
addEventHandler("onClientPlayerWasted", localPlayer,
	function (killer, weapon, bodypart)
		...
		preDeathCamera = {getCameraMatrix()}
		...
		setElementData(localPlayer, "tazed", false)
		setElementData(localPlayer, "player.seatBelt", false)
		setElementData(localPlayer, "deathState", true)

	setTimer(
		function()
			if getElementData(localPlayer, "deathState") then
				setElementData(localPlayer, "deathState", false)
				print("reset")
			end
		end, 15000, 1)
	...
	function ()
		...
		if currentHealth > 0 and currentHealth <= 20 then
			...
			if not animControls then
				...
				setElementData(localPlayer, "startedAnim", 1)
			...
			if endAnimTimer then
				...
				if minute < 5 then
					dxDrawBorderText(string.format("%02d:%02d", minute, second), 0, screenY - screenY / 5, screenX, screenY, tocolor(215, 89, 89), 1, Rubik, "center", "center")
				else
					dxDrawBorderText(string.format("%02d:%02d", minute, second), 0, screenY - screenY / 5, screenX, screenY, tocolor(255, 255, 255), 1, Rubik, "center", "center")
				end
			end
		...
		if preDeathStart then
			...
			local x, y, z, x2, y2, z2, roll, fov = unpack(preDeathCamera)

			if elapsedTime <= 2000 then
				x, y, z = interpolateBetween(x, y, z, x2, y2, z2, elapsedTime/2000, "InOutQuad")
				x2, y2, z2 = interpolateBetween(x2, y2, z2, playerX, playerY, playerZ, elapsedTime/2000, "InOutQuad")
			elseif elapsedTime > 2000 and elapsedTime <= 4000 then
				x, y, z = interpolateBetween(x2, y2, z2, playerX, playerY, playerZ + 5, (elapsedTime-2000)/2000, "OutInQuad")
				x2, y2, z2 = playerX, playerY, playerZ
			elseif elapsedTime > 4000 then
				local endX, endY, endZ = playerX, playerY, playerZ + 60
				if getElementInterior(localPlayer) ~= 0 then
					endZ = getRoofPosition(endX, endY, endZ)
				end

				x, y, z = interpolateBetween(playerX, playerY, playerZ + 5, endX, endY, endZ, (elapsedTime-4000)/30000, "InOutQuad")
				x2, y2, z2 = playerX, playerY, playerZ

				roll = (select(3, getElementRotation(getCamera())) - 180) + 180
			end

			setCameraMatrix(x, y, z, x2, y2, z2, roll, fov)
			dxUpdateScreenSource(screenSource)
			dxDrawImage(0, 0, screenX, screenY, deathShader, 0, 0, 0, tocolor(255, 255, 255, 255))
		...
		elseif deathStart then
			...
			local timeLeft = (300000 - elapsedTime) / 1000
			local minute = timeLeft / 60 % 60
			local second = timeLeft % 60
			...
			if not deathEnd then
				...
				if minute < 5 then
					dxDrawBorderText(string.format("%02d:%02d", minute, second), 0, 0, screenX - 50, screenY - 50, tocolor(215, 89, 89), 1, Rubik, "right", "bottom")
				else
					dxDrawBorderText(string.format("%02d:%02d", minute, second), 0, 0, screenX - 50, screenY - 50, tocolor(255, 255, 255), 1, Rubik, "right", "bottom")
				end
			end
		end
