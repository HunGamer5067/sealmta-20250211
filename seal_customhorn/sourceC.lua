	...
	function ()
		...
		if pedveh then
			if getVehicleController(pedveh) == localPlayer then
				...
				if hornId >= 1 then
					exports.seal_controls:toggleControl({"horn"}, false)
...
addEventHandler("onClientVehicleExit", getRootElement(),
	function (exitPlayer)
		if customHorn and exitPlayer == localPlayer then
			customHorn = false
			exports.seal_controls:toggleControl({"horn"}, true)
		...
		if dataName == "hornState" then
			if getElementData(source, dataName) then
				if isElementStreamedIn(source) then
					...
					if hornId >= 1 then
						hornSounds[source] = playSound3D("horns/" .. hornId .. ".wav", 0, 0, 0, true)
						setSoundEffectEnabled(hornSounds[source], "i3dl2reverb", true)
