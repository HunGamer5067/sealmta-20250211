...
function stopMinigame()
	if minigameData then
		if minigameState == "ghero" then
			...
			if isElement(minigameData.RubikFont) then
				destroyElement(minigameData.RubikFont)
				minigameData.RubikFont = nil
			end
		end
	end
...
function startMinigame(gameType, successEvent, failEvent, ...)
	...
	if gameType == "ghero" then
		...
		minigameData.RubikFont = dxCreateFont("files/Rubik.ttf", 24, false, "proof")
