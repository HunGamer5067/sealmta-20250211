...
function setSpeedCamRender()
    if staticTempObject then
        ...
        dxDrawText("Az E betű megnyomásával lerakhatod a traffipaxot.", screenX / 2 - 250, screenY - 175, screenX / 2 + 250, screenY, tocolor(255, 255, 255, 200), 1, Roboto, "center", "center")
        ...
        if not isPedInVehicle(localPlayer) and getElementHealth(localPlayer) > 0 then
            if not staticTempObject and isOfficer() then
                if (getElementData(localPlayer, "speedCameras") or 0) < 1 then
                    ...
                    addEventHandler("onClientRender", getRootElement(), setSpeedCamRender, true, "low-9999999")
    ...
    function()
        if playerUseSpeedCam then
            ...
            if cursorX and cursorY then
                ...
                dxDrawText(((seatBelt == 1 and "[ÖV] ") or "") .. "Limit: " .. speedCamLimit .. "KM/h " .. 1900 + getRealTime().year .. "." .. getRealTime().month .. "." .. getRealTime().monthday .. " " .. getRealTime().hour .. ":" .. getRealTime().minute .. ":" .. getRealTime().second, screenX / 2 - 200, screenY - 75, screenX / 2 + 200, screenY, tocolor(255, 255, 255), 1, LCDFont, "center", "center")
            end
...
function isOfficer()
    return exports.seal_groups:isPlayerOfficer(localPlayer)
end
...
addEventHandler("onClientRender", getRootElement(), 
    function()
        if nearbyCustomSpeedCam then
            ...
            dxDrawText("Az E betű megnyomásával felveheted a traffipaxot.\nAz /settraffi paranccsal beállíthatod a traffipaxot.\nJobbklikk a traffipaxra a használathoz.", screenX / 2 - 250, screenY - 175, screenX / 2 + 250, screenY, tocolor(255, 255, 255, 200), 1, Roboto, "center", "center")
        end
    end, true, "low-9999999"
)
...
function speedCameraClick(sourceKey, keyState, absX, absY, worldX, worldY, worldZ, clickedElement)
    if sourceKey == "right" and keyState == "up" and clickedElement and getElementData(clickedElement, "isCustomSpeedCam") and isOfficer() then
        if getElementData(clickedElement, "inUse") then
            outputChatBox("#4adfbf[Traffipax]:#FFFFFF Valaki már használja!", 255, 255, 255, true)
            return
        end
        ...
        if getElementData(clickedElement, "limit") then
            ...
            outputChatBox("#4adfbf[Traffipax]:#FFFFFF Ez a traffipax nincs beállítva!", 255, 255, 255, true)
        end
    end
...
addEventHandler("speedAlertFromServer", getRootElement(), 
    function(vehSpeed, speedLimit, speedTicket, beltInfo)
        if vehSpeed and speedLimit and speedTicket then
            ...
            outputChatBox("#4adfbf[SealMTA - Traffipax]: #FFFFFFBemértek! Büntetésed: #d75959" .. speedTicket .. "$", 0, 0, 0, true)
            outputChatBox("#4adfbf[SealMTA - Traffipax]: #FFFFFFSebességed: #4adfbf" .. vehSpeed .. " KM/h#FFFFFF, Limit: #4adfbf" .. speedLimit .. "KM/h", 0, 0, 0, true)
            
            if beltInfo then
                outputChatBox("#4adfbf[SealMTA - Traffipax]: #FFFFFFNem volt bekötve " .. beltInfo .. " személy az öve a járművedben. Büntetés: #d75959" .. (beltInfo * 50) .. "$", 0, 0, 0, true)
            end
    ...
    function(cmd, speedLimit, seatBelt)
        if nearbyCustomSpeedCam and not getElementData(localPlayer, "usingSpeedCamera") and isOfficer() then
            ...
            if not speedLimit or speedLimit < 30 or speedLimit > 130 or seatBelt > 1 then
                return outputChatBox("#4adfbf[Traffipax]:#FFFFFF Használat: /" .. cmd .. " [limit 30-130khm/h] [Öv ki: 0 be: 1]", 255, 255, 255, true)
            end
            ...
            outputChatBox("#4adfbf[Traffipax]:#FFFFFF Beállítva.", 255, 255, 255, true)
        end
    end
)
