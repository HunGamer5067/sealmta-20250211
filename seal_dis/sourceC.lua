...
addEventHandler("onEarthquake", getRootElement(), function(earthQuakeTime)
	...
	setTimer(setCameraDrunkLevel, 1000 * (earthQuakeTime + 1), 1, 0)
	setTimer(setPedControlState, 1000 * (earthQuakeTime + 1), 1, "left", false)
	setTimer(setPedControlState, 1000 * (earthQuakeTime + 1), 1, "right", false)
...
addCommandHandler("villam", function()
	if getElementData(localPlayer, "acc.adminLevel") >= 9 and enabledSerials[getElementData(localPlayer, "char.accID")] then
	    boltActive = not boltActive
	    outputChatBox("#bc873d[WeakMTA]:#ffffff Villám mód " .. (boltActive and "#bc873don" or "#d75959off") .. ".", 255, 255, 255, true)
	end
end)
