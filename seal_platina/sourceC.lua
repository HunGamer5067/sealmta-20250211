...
addEventHandler("onClientElementDataChange", getRootElement(),
	function (dataName, oldData, newData)
		...
		elseif dataName == "vehicleColor" then
			replaceColor(source)
		elseif dataName == "animated_index" then
			print(newData)
			...
			shaders[source] = nil
				...
				for _, v in ipairs(getElementsByType("vehicle")) do
					if isElement(shaders[v]) then
						destroyElement(shaders[v])
					end
				end
			elseif newData == true then
				for _, v in ipairs(getElementsByType("vehicle")) do
					if getElementData(v, "vehicle.Tuning.Platina") then
						applyTexture(v)
					end
				end
			end
...
--dashboard.platinaTurned
function applyTexture(vehicle)
	if getElementData(localPlayer, "animated_index") then
