	...
	function (sourcePlayer, commandName, databaseId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 7 then
			...
			if not databaseId then
				outputChatBox("#4adfbf[Használat]: #ffffff/" .. commandName .. " [ID]", sourcePlayer, 255, 255, 255, true)
			else
				...
				if isElement(objectElement) then
					outputChatBox("#4adfbf[SealMTA]: #ffffffSikeresen törölted a kiválasztott ATM-et. #32b3ef(" .. databaseId .. ")", sourcePlayer, 255, 255, 255, true)
					...
					outputChatBox("#d75959[SealMTA]: #ffffffA kiválasztott ATM nem létezik.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
...
addEventHandler("placeATM", getRootElement(),
	function (positionsTable)
		if isElement(source) and client == source and getElementData(source, "acc.adminLevel") >= 6 then
	...
	if resultRows then
		...
		if resultRow then
			...
			if isElement(sourcePlayer) then
				...
				exports.seal_gui:showInfobox(sourcePlayer, "s", "ATM sikeresen létrehozva! ID: " .. resultRow.dbID)
			end
		end
	end
end
...
function resetRobbedATMs()
	for k in pairs(atmDataTable) do
		if getElementModel(k) == 2943 or getElementModel(k) == 2942 then
...
addCommandHandler("resetatm",
	function(sourcePlayer)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
			resetRobbedATMs()
			outputChatBox("#4adfbf[SealMTA]: #ffffffSikeresen visszaállítottad az ATM-eket!", sourcePlayer, 255,255,255,true)
		end
	end
)
...
addEventHandler("openATMCompartment", getRootElement(),
	function (playersTable, objectElement, compartmentId)
		if isElement(source) and client == source then
			if atmDataTable[objectElement] then
				if exports.seal_items:hasSpaceForItem(source, 119, 1) then
					triggerClientEvent(playersTable, "atmCompartmentSound", source, objectElement, compartmentId)

					exports.seal_items:giveItem(source, 119, 1)
					...
					exports.seal_gui:showInfobox(source, "e", "Nem fér el nálad a tárgy!")
				end
			end
		end
	end
				...
				if amount then
					...
					if amount then
						...
						if exports.seal_core:takeMoneyEx(source, amount) then
							...
							exports.seal_gui:showInfobox(source, "s", "Sikeres tranzakció! Részletek a chatboxban!")
						end
					end
				end
			end
...
addEventHandler("giveCasetteMoney", getRootElement(),
	function(amount)
		if isElement(source) and client and client == source and exports.seal_items:hasItem(client, 119, 1) and exports.seal_items:hasItem(client, 120, 1) and exports.seal_items:hasItem(client, 121, 1) then
