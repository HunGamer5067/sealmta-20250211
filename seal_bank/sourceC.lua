...
addCommandHandler("nearbyatm",
	function (commandName)
		if getElementData(localPlayer, "acc.adminLevel") >= 7 then
			...
			outputChatBox("#4adfbf------[Közeledben lévő ATM-ek]------", 255, 255, 255, true)
...
function renderATMPlacement()
	if placedATM then
		local currentX, currentY, currentZ = getPositionFromElementOffset(localPlayer, 0, 0.0, -0.5)
											...
											if amount > 0 then
												if activeButton == "withdraw" then
													if amount <= currentBalance then
														...
														exports.seal_gui:showInfobox("e", "Nincs ennyi pénz a bankszámládon!")
													end
												elseif activeButton == "deposit" then
													if amount <= currentMoney then
														triggerServerEvent("depositMoney", localPlayer, amount)
													else
														exports.seal_gui:showInfobox("e", "Nincs ennyi pénz nálad!")
													end
												end
											else
												exports.seal_gui:showInfobox("e", "Inkább maradjunk az egész, nullánál nagyobb számoknál!")
											end
										else
											exports.seal_gui:showInfobox("e", "Nem gondolod, hogy meg kellene adnod valamilyen számot?")
										end
									else
										exports.seal_gui:showInfobox("e", "Azért ennyire ne legyél vagány, hogy járműből akard használni a panelt!")
									end
								else
									exports.seal_gui:showInfobox("e", "Na, na! Ott használd ahol megnyitottad a panelt!")
								end
							end

							clickTick = getTickCount()
						else
							exports.seal_gui:showInfobox("e", "Csak 5 másodpercenként használhatod ezt a funkciót!")
						end
					end
				end
			end
...
function setGrinding(newState)
	if newState then
		if getTickCount() - clickTick < 1000 then
			exports.seal_gui:showInfobox("e", "Várj egy kicsit!")
			return
		end
		...
		if count < 4 then
			exports.seal_gui:showInfobox("e", "Minimum 4 rendvédelmis kell az ATM rablásához! (/getpdcount)")
			return
		end
	...
	if newState ~= grindingState then
		...
		if newState then
			exports.seal_chat:localActionC(localPlayer, "elkezdett flexelni.")
		else
			exports.seal_chat:localActionC(localPlayer, "befejezte a flexelést.")
		end
					...
					if press then
						...
						for i = 1, #playersTable do
							...
							if isElement(playerElement) then
								if exports.seal_groups:isPlayerInGroup(playerElement, {1, 2, 4}) then
						...
						if officerCount < minimumOfficer then
							exports.seal_gui:showInfobox("e", "Minimum " .. minimumOfficer .. " darab online rendvédelmis szükséges a rabláshoz.")
							return
						end

						if not exports.seal_groups:isPlayerHavePermission(localPlayer, "canRobATM") then
							exports.seal_gui:showInfobox("e", "Csak illegális frakciók tagjai tudják kirabolni az ATM-et.")
							return
						end
					end
					...
					if atmRobID then
						if currentATM then
							...
							if animControls then
								exports.seal_controls:toggleControl({"fire", "jump", "crouch", "sprint", "aim_weapon", "enter_exit"}, true)
...
function robStartATMRender()
	grindingProgress = math.floor(grindingATMHealth - (getTickCount() - grindingTick) / 100)
	...
	local playerX ,playerY, playerZ = getElementPosition(localPlayer)
	local distance = getDistanceBetweenPoints3D(currentATMPosition[1], currentATMPosition[2], currentATMPosition[3] + 0.5, playerX ,playerY, playerZ)

	if guiX and guiY and distance < 5 then
		dxDrawRectangle(guiX - 80 - 4, guiY - 4, 168, 18, tocolor(0, 0, 0, 150))
		dxDrawRectangle(guiX - 80, guiY, 160, 10, tocolor(0, 0, 0, 150))
		dxDrawRectangle(guiX - 80, guiY, 160 * (grindingProgress / atmDefaultHealth), 10, tocolor(94, 193, 230))
	end
end
		...
		if source == localPlayer then
			...
			if state then
				if not animControls then
					...
					exports.seal_controls:toggleControl({"fire", "jump", "crouch", "sprint", "aim_weapon", "enter_exit"}, false)
...
function clickATMRobGui(button, state, absX, absY)
	if state == "up" then
		...
		if exports.seal_core:isInSlot(x + 510 - 20, y, 20, 20) then
			if animControls then
				exports.seal_controls:toggleControl({"fire", "jump", "crouch", "sprint", "aim_weapon", "enter_exit"}, true)
		...
		if getTickCount() - clickTick > 2000 then
			...
			if not atmRobCompartments[1] and exports.seal_core:isInSlot(x + 6, y + 38, 242 - 4, 71 - 4) then
				triggerServerEvent("openATMCompartment", localPlayer, playersTable, currentATMObject, 1)
				exports.seal_chat:localActionC(localPlayer, "kivesz egy pénzkazettát az ATM-ből.")
			end

			if not atmRobCompartments[2] and exports.seal_core:isInSlot(x + 6, y + 122, 242 - 4, 79 - 4) then
				triggerServerEvent("openATMCompartment", localPlayer, playersTable, currentATMObject, 2)
				exports.seal_chat:localActionC(localPlayer, "kivesz egy pénzkazettát az ATM-ből.")
			end

			if not atmRobCompartments[3] and exports.seal_core:isInSlot(x + 264, y + 38, 242 - 4, 71 - 4) then
				triggerServerEvent("openATMCompartment", localPlayer, playersTable, currentATMObject, 3)
				exports.seal_chat:localActionC(localPlayer, "kivesz egy pénzkazettát az ATM-ből.")

			end

			if not atmRobCompartments[4] and exports.seal_core:isInSlot(x + 264, y + 122, 242 - 4, 79 - 4) then
...
addEventHandler("startMoneyCasetteOpen", getRootElement(),
	function ()
		if exports.seal_groups:isPlayerHavePermission(localPlayer, "canRobATM") then
			setMinigameState(true)
			exports.seal_chat:localActionC(localPlayer, "elkezd felfeszíteni egy pénzkazettát.")
		end
	end
)
...
function balanceMinigameFail()
	...
	exports.seal_chat:sendLocalDoC(localPlayer, "A pénzkazetta felrobbant.")
	...
	outputChatBox("#d75959[SealMTA - ATM]: #ffffffNem sikerült kinyitni a kazettát, ezért #d75959felrobbant a festékpatron#ffffff.", 255, 255, 255, true)
	outputChatBox("#d75959[SealMTA - ATM]: #ffffffA #943cb8festék#ffffff 3 órán át látható lesz az arcodon.", 255, 255, 255, true)
end
...
function balanceMinigameSuccess()
	...
	local casetteMoney = math.random(0, 135000)
	...
	if casetteMoney == 0 then
		outputChatBox("#4adfbf[SealMTA - ATM]: #ffffffKinyitottad a kazettát, de sajnos #d75959üres#ffffff volt.", 255, 255, 255, true)
	else
		outputChatBox("#4adfbf[SealMTA - ATM]: #ffffffKinyitottad a kazettát, és #4adfbf" .. casetteMoney .. " $#ffffff volt benne.", 255, 255, 255, true)
	end

	exports.seal_chat:sendLocalDoC(localPlayer, "A pénzkazetta kinyílt.")
