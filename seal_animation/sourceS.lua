...
local setPedAnimation = function(playerElement, block, anim, time, loop, updatePoisition, interruptable, freezeLastFrame)
	if canUseAnimation(playerElement) then
		return _setPedAnimation(playerElement, block, anim, time, loop, updatePoisition, interruptable, freezeLastFrame)
	end
end
...
function animationList(thePlayer)
	outputChatBox("/piss /wank /slapass /fixcar /handsup /hailtaxi /scratch /fu /carchat /tired /kiss", thePlayer, 255, 194, 14)
	...
	outputChatBox("/stopchat /baseball /buydrog /kiss1, /int ", thePlayer, 255, 194, 14)
	outputChatBox("/stopanim vagy space az animáció megállításához!", thePlayer, 255, 194, 14)
end
...
function coverAnimation(thePlayer)
	local logged = getElementData(thePlayer, "loggedIn") and not getPedOccupiedVehicle(thePlayer)

	if not (logged) then
	exports.seal_accounts:showInfo(thePlayer, "e", "Nem használhatod!")
else
...
-- /csók animtion -------------------------------------------------
function kiss(thePlayer)
	local logged = getElementData(thePlayer, "loggedIn") and not getPedOccupiedVehicle(thePlayer)
	
	if not (logged) then
	exports.seal_accounts:showInfo(thePlayer, "e", "Nem használhatod!")
else
		setPedAnimation( thePlayer, "KISSING", "grlfrd_kiss_03", 8000, false, true, false)
	end
end
addCommandHandler("kiss", kiss, false, false)

-- /csók animtion -------------------------------------------------
function kiss1(thePlayer)
	local logged = getElementData(thePlayer, "loggedIn") and not getPedOccupiedVehicle(thePlayer)
	
	if not (logged) then
	exports.seal_accounts:showInfo(thePlayer, "e", "Nem használhatod!")
else
		setPedAnimation( thePlayer, "KISSING", "grlfrd_kiss_02", 8000, false, true, false)
	end
end
addCommandHandler("kiss1", kiss1, false, false)
...
function pedMourn( thePlayer )
	...
	exports.seal_accounts:showInfo(thePlayer, "e", "Nem használhatod!")
		...
		setPedAnimation( thePlayer, "GRAVEYARD", "mrseal_loop", -1, true, false, false)
	end
end
...
-- Intés animáció ---------------------------------------------------------------------------------
function int(thePlayer, cmd, arg)
	local logged = getElementData(thePlayer, "loggedIn") and not getPedOccupiedVehicle(thePlayer)

	if not (logged) then
	exports.seal_accounts:showInfo(thePlayer, "e", "Nem használhatod!")
else
		setPedAnimation( thePlayer, "KISSING", "gfwave2", 2000, false, false, false)
	end
end
addCommandHandler ( "int", int, false, false )
