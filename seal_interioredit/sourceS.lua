...
function loadInterior(thePlayer, interiorId, editInterior, forceLoad)
	...
	local editable = exports.seal_interiors:getInteriorEditable(interiorId)
...
registerEvent("interiorEdit:loadInterior", getRootElement(),
	function (interiorId, editInterior)
		if isElement(source) and interiorId and client == source then
	...
	function (normalexit)
		if isElement(source) and client == source then
			...
			if interiorId > 0 then
				...
				if normalexit then
					local entrance = exports.seal_interiors:getInteriorEntrance(interiorId)

					exports.seal_interiors:warpElement(source, entrance.position[1], entrance.position[2], entrance.position[3], entrance.rotation[1], entrance.rotation[2], entrance.rotation[3], entrance.interior, entrance.dimension)
...
registerEvent("interiorEdit:saveInterior", getRootElement(),
	function (interiorId, interiorData, costs)
		if isElement(source) and client == source and getElementDimension(client) == interiorId then
			if interiorId and interiorData then
				local entrance = exports.seal_interiors:getInteriorEntrance(interiorId)
				local currCosts = costs - interiorDatas[interiorId].paidCash

				if currCosts > 0 then
					exports.seal_core:takeMoney(source, currCosts, eventName)
				else
					exports.seal_core:giveMoney(source, currCosts, eventName)
				end
	...
	function (interiorId, model)
		if source == client and interiorId and model then
			...
			if currBalance >= 0 then
				...
				exports.seal_gui:showInfobox(source, "s", "Sikeres vásárlás!")
			else
				exports.seal_gui:showInfobox(source, "e", "Nincs elég prémium pontod!")
			end
		end
	end)
...
registerEvent("interiorEdit:requestTvMovie", getRootElement(),
	function ()
		if isElement(source) and getElementDimension(client) == interiorId then
...
registerEvent("interiorEdit:playTvMovie", getRootElement(),
	function (url, interiorId, fromMovieList)
		if isElement(source) and url and interiorId and getElementDimension(client) == interiorId then
			...
			if url and id then
				...
				exports.seal_hud:showInfobox(client, "e", "Érvénytelen URL! Kérlek próbálj másikat.")
			end
		end
	end)