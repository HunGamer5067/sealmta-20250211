...
local photoSizeX = 160 * 1.2
local photoSizeY = 400 * 1.2
		...
		if photoMode then
			...
			if modelId then
				if not isValidModel(modelId) then
					exports.seal_accounts:showInfo("e", "A kiválasztott model ID érvénytelen!")
					return
				end
			end
			...
			fontElement = dxCreateFont(":seal_hud/files/fonts/Roboto.ttf", 14, false, "antialiased")
	...
	threadTimer = setTimer(
		function()
			...
			elseif status == "dead" then
				killTimer(threadTimer)
				captureThread = nil
				exports.seal_accounts:showInfo("s", "A képek generálása befejeződött.")
			end
		end,
	timeInterval, 0)
end
...
function captureTheScreen(modelId)
	...
	local texturPixels = dxGetTexturePixels(0, renderTarget, photoPosX, photoPosY, photoSizeX, photoSizeY)
