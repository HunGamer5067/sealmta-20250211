			...
			if getPedOccupiedVehicleSeat(localPlayer) == 0 then
				...
				if wiperLookup[model] then
					if getElementData(currentVehicle, "wiperState") then
						...
						exports.seal_chat:localActionC(localPlayer, "kikapcsolja az ablaktörlőt.")
					else
						setElementData(currentVehicle, "wiperState", true)
						exports.seal_chat:localActionC(localPlayer, "bekapcsolja az ablaktörlőt.")
					end
				end
			end
		end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		for i = 1, #vehicles do
			...
			if headlightEnabled[model] then
				local comp = "FrontLights2"
				local degree = tonumber(headlightEnabled[model]) or 57.5
				local x1, y1, z1 = getVehicleComponentRotation(vehicle, "FrontLights2")
				if not x1 or model == 477 then
					comp = "misc_a"
					x1, y1, z1 = getVehicleComponentRotation(vehicle, comp)
				end

				if x1 then
					if getVehicleOverrideLights(vehicle) == 2 then
						if compRotX ~= degree then
							if not headlightAnimation[vehicle] or not headlightAnimation[vehicle][4] then
								headlightAnimation[vehicle] = {currentTick, math.abs(degree - x1) / degree * 650, x1, true}
							end
							...
							local x = interpolateBetween(headlightAnimation[vehicle][3], 0, 0, degree, 0, 0, progress, "InOutQuad")
				
							setVehicleComponentRotation(vehicle, comp, x, y1, z1)
						end
					else
						if x1 ~= 0 then
							if not headlightAnimation[vehicle] or headlightAnimation[vehicle][4] then
								headlightAnimation[vehicle] = {currentTick, math.abs(0 - x1) / degree * 650, x1, false}
							end
