...
table.insert(availableJobs[1], {1308.767578125, -872.20739746094, 39.578125, 498, -1, "A postai csomagok és levelek szállítására szolgáló jármű.", false, "#ffffff", false})

table.insert(availableJobs[2], {215.16400146484, -173.45170593262, 1.578125, 448, -1, "A pizza szállítására szolgáló robogó.", false, "#8b1014", false})

table.insert(availableJobs[3], {2179.3999, -2286.2, 13.5, 431, -1, "Tömegközlekedési jármű, az utasok szállítására alkalmas.", false, "#7CC576", false})
table.insert(availableJobs[3], {2179.3999, -2286.2, 13.5, 431, -1, "Tömegközlekedési jármű, az utasok szállítására alkalmas.", false, "#d75959", false})
...
addCommandHandler("nearbyjobvehicles",
	function ()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			outputChatBox("#4adfbf[SealMTA]: #FFFFFFA közeledben lévő munkajárművek (15 yard):", 255, 255, 255, true)
			...
			for k, v in pairs(getElementsByType("vehicle", getRootElement(), true)) do
				...
				if id then
					...
					if dist <= 15 then
						outputChatBox("#4adfbf[SealMTA]: #FFFFFFAzonosító: #8dc63f" .. id .. "#FFFFFF <> Típus: #8dc63f" .. exports.seal_vehiclenames:getCustomVehicleName(getElementModel(v)) .. "#FFFFFF <> Távolság: #8dc63f" .. math.floor(dist), 255, 255, 255, true)
						nearby = nearby + 1
					end
				end
			end
			...
			if nearby == 0 then
				outputChatBox("#4adfbf[SealMTA]: #FFFFFFA közeledben nem található egyetlen munkajármű sem.", 255, 255, 255, true)
			end
		end
	end)
		...
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			for k, v in pairs(getElementsByType("vehicle", getRootElement(), true)) do
				if vehicleId == getElementData(v, "jobVehicleID") then
					...
					outputChatBox("#4adfbf[SealMTA]: #FFFFFFA munkajármű sikeresen törölve.", 255, 255, 255, true)
					found = true
					break
				end
			end
			...
			if not found then
				outputChatBox("#d75959[SealMTA]: #FFFFFFNincs találat.", 255, 255, 255, true)
			end
		end
	end)
...
addCommandHandler("getjobowner",
	function (commandName, vehicleId)
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			for k, v in pairs(getElementsByType("vehicle", getRootElement(), true)) do
				if vehicleId == getElementData(v, "jobVehicleID") then
					outputChatBox("#4adfbf[SealMTA - Munkajármű]: #FFFFFFLehívó: " .. (getElementData(v, "jobSpawnerName") or "N/A") .. " (" .. (getElementData(v, "jobSpawner") or "N/A") .. ")", 255, 255, 255, true)
					found = true
					break
				end
			end
			...
			if jobData[source] then
				...
				if isElement(pedveh) then
					if getElementModel(pedveh) == jobData[source][1] or getElementModel(pedveh) == jobData[source][2] then
						...
						outputChatBox("#4adfbf[SealMTA]: #FFFFFFEzt a járművet itt nem tudod leadni.", 255, 255, 255, true)
					end
				else
					Roboto = dxCreateFont("files/Roboto.ttf", 20)
					panelState = true
					...
					selectedJob[5] = exports.seal_vehiclenames:getCustomVehicleName(selectedJob[1])
					selectedJob[6] = selectedJob[2] ~= -1 and exports.seal_vehiclenames:getCustomVehicleName(selectedJob[2])
	...
	function ()
		if panelState == "destroy" then
			...
			if activeButton == "close" then
				dxDrawRectangle(x + sx - 170, y + sy - 50, 160, 40, tocolor(94, 193, 230))
			else
				dxDrawRectangle(x + sx - 170, y + sy - 50, 160, 40, tocolor(94, 193, 230, 200))
			end
		...
		elseif panelState then
			...
			if selectedJob[4] then
				...
				if activeButton == "spawn" then
					dxDrawRectangle(x + 10, y + sy - 100, 240, 40, tocolor(94, 193, 230))
				else
					dxDrawRectangle(x + 10, y + sy - 100, 240, 40, tocolor(94, 193, 230, 200))
				end

				if activeButton == "spawn2" then
					dxDrawRectangle(x + sx - 248, y + sy - 100, 240, 40, tocolor(94, 193, 230))
				else
					dxDrawRectangle(x + sx - 248, y + sy - 100, 240, 40, tocolor(94, 193, 230, 200))
				end
				...
				if activeButton == "spawn" then
					dxDrawRectangle(x + 10, y + sy - 100, sx - 20, 40, tocolor(94, 193, 230))
				else
					dxDrawRectangle(x + 10, y + sy - 100, sx - 20, 40, tocolor(94, 193, 230, 200))
				end
