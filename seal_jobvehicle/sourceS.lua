	...
	function (vehicle)
		...
		if vehicleId then
			...
			if spawnerName == myName then
				...
				exports.seal_hud:showInfobox(source, "i", "Beszálltál a munkajárművedbe.")
			end
		end
	end)
		...
		if not forcedByScript then
			...
			if vehicleId then
				...
				if spawnerName == myName then
					...
					exports.seal_hud:showInfobox(source, "i", "Kiszálltál a munkajárművedből. Ha nem szállsz vissza, a jármű 15 percen belül törlődik!")
				end
			end
		end
	end)
...
addEventHandler("&ÄäÄí{@ÄÍ&ŁÄ", getRootElement(),
	function (modelId, colorCode)
		if isElement(source) and client and client == source then
			...
			if not modelId then
				exports.seal_hud:showInfobox(source, "i", "Sikeresen leadtad a munkajárműved.")
			...
			elseif isElement(jobVehicles[source]) then
				exports.seal_hud:showInfobox(source, "e", "Már van egy munkajárműved!")
			else
				...
				if characterJob > 0 then
					...
					if isElement(vehicleElement) then
						...
						if vehicleId then
							...
							exports.seal_tuning:applyHandling(vehicleElement)
							...
							setElementData(vehicleElement, "vehicle.fuel", exports.seal_vehiclepanel:getTheFuelTankSizeOfVehicle(modelId))
							...
							setVehiclePlateText(vehicleElement, "MUNKA-" .. vehicleId)
							...
							outputChatBox("#dc143c[SealMTA]: #ffffffSikeresen lehívtál egy munkajárművet. Amennyiben le szeretnéd adni, hozd ide.", source, 0, 0, 0, true)

							warpPedIntoVehicle(source, vehicleElement, 0)
