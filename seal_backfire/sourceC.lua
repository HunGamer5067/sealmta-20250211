...
function getElementSpeed(currentElement)
...
addEventHandler("onDiesel", getRootElement(), 
	function()
		if isElement(source) and source then
			if getElementData(source, "extraBackFireTuning") then
				local s = playSound3D("sound/bfnew.MP3", getElementPosition(source))
			...
			if sound and speed and consistence then
				...
				if getElementData(source, "extraBackFireTuning") then
					for k = 1, 15 do
						...
						local exhaustX, exhaustY, exhaustZ = getVehicleModelExhaustFumesPosition((getElementModel(source)))
						table.insert(vehBackfires, {
							fxCreateParticle("huge_smoke", vehX, vehY, vehZ, 0, 0, 0, 20, 20, 20, 255, false, 1, 1, 1, false, 0.3),
							source,
							getTickCount(),
							exhaustX,
							exhaustY,
					...
					for k = 1, 15 do
						...
						local exhaustX, exhaustY, exhaustZ = getVehicleModelExhaustFumesPosition((getElementModel(source)))
						table.insert(vehBackfires, {
							fxCreateParticle("sand", vehX, vehY, vehZ, 0, 0, 0, 20, 20, 20, 255, false, 1, 1, 1, false, 0.3),
							source,
							getTickCount(),
							exhaustX,
							exhaustY,
...
local lastBackfireTick = 0
local backfireCooldown = 2000
...
function processBackfire()
    for k = 1, #vehBackfires do
        if vehBackfires[k] and vehBackfires[k][8] then
            ...
            if isElement(vehBackfires[k][2]) then
                ...
                dxDrawMaterialSectionLine3D(x + rx * sp, y + ry * sp, z + rz * sp, x + rx * (s * (1 - p2)), y + ry * (s * (1 - p2)), z + rz * (s * (1 - p2)), math.random(0, 7) * 128, 256 * (1 - p), 128, 256 * p * p2, fireTex, s / 2, tocolor(255, 255, 255, 200), false)
    ...
    if getPedOccupiedVehicle(localPlayer) and getPedOccupiedVehicle(localPlayer) ~= vehGearSecond then
        vehicleTuning = 0
    ...
    local vehicleSpeed = getElementSpeed(vehicle)
    if vehicle and getPedOccupiedVehicleSeat(localPlayer) == 0 and vehicleTuning >= 16 then
        local isAccelerating = getPedControlState(localPlayer, "accelerate")

        if not isAccelerating and isGasPressed and vehicleSpeed > 50 then
            local currentTick = getTickCount()
            if currentTick - lastBackfireTick >= backfireCooldown then
                lastBackfireTick = currentTick
                ...
                if getElementData(vehicle, "extraBackFireTuning") then
                	minBackfire = 1
                	maxBackfire = 20
                	...
                	local backfireCount = math.floor(minBackfire + (math.min(vehicleSpeed, maxSpeed) - minSpeed) / (maxSpeed - minSpeed) * (maxBackfire - minBackfire))
	                backfireCount = math.max(minBackfire, math.min(maxBackfire, backfireCount))

	                local dieselCount = math.floor(minDiesel + (math.min(vehicleSpeed, maxSpeed) - minSpeed) / (maxSpeed - minSpeed) * (maxDiesel - minDiesel))
	                dieselCount = math.max(minDiesel, math.min(maxDiesel, dieselCount))

	                local function triggerBackfire(index)
	                    if index > backfireCount then return end
	                    triggerServerEvent("onBackFire", vehicle, getElementsByType("player", getRootElement(), true))
	                    setTimer(function()
	                        triggerBackfire(index + 1)
	                    end, 125, 1)
	                end

	                local function triggerDiesel(index)
	                	if index > dieselCount then return end
	                	triggerServerEvent("onDiesel", vehicle, getElementsByType("player", getRootElement(), true))
	                	setTimer(function()
	                		triggerDiesel(index + 1)
	                	end, 125, 1)
	                end
	                triggerBackfire(1)
	                triggerDiesel(1)
...
addCommandHandler("launchcontrol", function(playerSource, commandName, ...)
	local pedVehicle = getPedOccupiedVehicle(localPlayer)
	if getElementData(pedVehicle, "extraBackFireTuning") then
		triggerServerEvent("onLaunchControl", pedVehicle, getElementsByType("player", getRootElement(), true))
	end
end)

addEvent("onLaunchControl", true)
addEventHandler("onLaunchControl", getRootElement(), function (...)
	if isElement(source) then
		local x, y, z = getElementPosition(source)
		local s = playSound3D("sound/popcorn.mp3", x, y, z)
		setSoundVolume(s, 3)
		setElementDimension(s, getElementDimension(localPlayer))
		setElementInterior(s, getElementInterior(localPlayer))
		setSoundMaxDistance(s, 90)
	end
end)