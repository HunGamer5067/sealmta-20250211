...
function createFonts()
	destroyFonts()
	panelFont = dxCreateFont("files/Rubik.ttf", 12, false, "proof")
end
	...
	function ()
		for k, v in pairs(availableShops) do
			if v[1] and v[2] and v[3] and v[4] then
				local markerElement = createMarker(v[1], v[2], v[3], "cylinder", 2, 94, 193, 230, 155)
					...
					if currentMarkerType then
						...
						if markerShopId then
							if availableShops[markerShopId] then
								...
								exports.seal_gui:showInfobox("i", "Nyomj [E] gombot a kínálat megtekintéséhez.")
								inColShape = true
							end
						end
					end
	...
	function (key, press)
		if key == "e" and not press then
			if not panelState and inColShape then
				...
				selectedSkin = 1
				skinsTable = availableSkins[selectedShop[7]]

				if skinsTable and skinsTable[selectedSkin] and skinsTable[selectedSkin][1] then
					previewPed = createPed(skinsTable[selectedSkin][1], 202.71890258789, -112.01360321045, 1005.2578125, 0, false)
	...
	function ()
		if panelState then
			...
			if absX >= buttonPosX and absY >= buttonPosY and absX <= buttonPosX + buttonWidth and absY <= buttonPosY + buttonHeight then
				buttonColor = tocolor(94, 193, 230, 200)
				textColor = tocolor(0, 0, 0)
			end

			dxDrawRectangle(buttonPosX, buttonPosY, buttonWidth, buttonHeight, tocolor(25,25,25)) -- háttér
			drawButton("elozo", "Előző", buttonPosX + 4, buttonPosY + 4, buttonWidth - 8, buttonHeight - 8, {94, 193, 230}, false, panelFont, true)
			...
			dxDrawRectangle(buttonPosX, buttonPosY, buttonWidth, buttonHeight, tocolor(25,25,25)) -- háttér
			drawButton("kovetkezo", "Következő", buttonPosX + 4, buttonPosY + 4, buttonWidth - 8, buttonHeight - 8, {94, 193, 230}, false, panelFont, true)
			...
			dxDrawRectangle(buttonPosX, buttonPosY, buttonWidth, buttonHeight, tocolor(25,25,25)) -- háttér
			drawButton("buy", buttonText, buttonPosX + 4, buttonPosY + 4, buttonWidth - 8, buttonHeight - 8, {94, 193, 230}, false, panelFont, true)
			...
			setCameraMatrix(
				pedPosX + math.cos(math.rad(selectedShop[4] - 18)) * cameraZoom,
				pedPosY + math.sin(math.rad(selectedShop[4] - 18)) * cameraZoom,
				pedPosZ + 1.5,
				pedPosX,
				pedPosY,
				pedPosZ + cameraZ
					...
					elseif absoluteX >= screenX / 2 - 105 and absoluteX <= screenX / 2 + 105 and absoluteY >= screenY - 180 and absoluteY <= screenY - 120 then
						if skinsTable[selectedSkin] then
							if buySkinTick + 1000 <= getTickCount() then
								...
								outputChatBox("#d75959[SealMTA]: #ffffffMásodpercenként csak egyszer.", 0, 0, 0, true)
							end
						end
					end
				end
...
addEventHandler("bincoBuy", getRootElement(),
	function (skinID)
		outputChatBox("#4adfbf[SealMTA]: #ffffffVettél egy új ruhát." .. " Skin ID: #4adfbf".. skinID, 0, 0, 0, true)
		closePanel()
	end
)