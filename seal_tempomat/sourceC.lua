...
function endTempomat()
	...
	exports.seal_controls:toggleControl({"accelerate", "brake_reverse"}, true)
	...
	function ()
		...
		if vehicle and getPedOccupiedVehicleSeat(localPlayer) == 0 and getElementData(vehicle, "vehicle.engine") then
			...
			if speed > 5 then
				...
				exports.seal_controls:toggleControl({"accelerate", "brake_reverse"}, false)
				...
				setElementData(vehicle, "tempomatSpeed", math.floor(cruiseSpeed))
			end
		end
	end
)
