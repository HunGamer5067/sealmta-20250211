	...
	function (thePlayer, seat)
		if seat == 0 then
			if getElementModel(source) == 431 then
				...
				if (playerJob == 3 or playerJob == 3) and jobVehicle then
					if jobVehicle == source then
						if not activeJobs[thePlayer] then
							...
							activeJobs[thePlayer] = {
								...
								nextWp = 0,
								lastWpCheck = 0,
								lineType = playerJob == 3 and "normal" or "country",
								lineColor = playerJob == 3 and {94, 193, 230} or {215, 89, 89},
								peds = {},
								colshape = false
							}
...
function nextPoint(playerElement)
	if isElement(playerElement) then
		...
		if currentVehicle ~= jobVehicle then
			exports.seal_hud:showInfobox(playerElement, "e", "Nem ez a munkajárműved!")
			return
		end
		...
		local totalPrize = 5000 * passengers
		local playerPP = getElementData(playerElement, "acc.premiumPoints") or 0
		local newPP = math.random(25, 50)
		...
		if passengers > 0 then
			exports.seal_core:giveMoney(playerElement, totalPrize)
			setElementData(playerElement, "acc.premiumPoints", playerPP + newPP)
			outputChatBox("#4adfbf[SealMTA - Buszsofőr]: #ffffffÖsszesen felszállt #4adfbf" .. passengers .. " db#ffffff utas, és kerestél #4adfbf" .. totalPrize .. "$#ffffff-t és #4adfbf" .. newPP .. " prémiumpontot ezen a megállóhelyen.", playerElement, activeJob.lineColor[1], activeJob.lineColor[2], activeJob.lineColor[3], true)
		end
						...
						if hitElement == getVehicleController(currentVehicle) then
							...
							if activeJob then
								if getVehicleSpeed(currentVehicle) <= 20 then
									...
									exports.seal_hud:showInfobox(hitElement, "e", "Ilyen gyorsan nem tudsz utasokat felvenni. Lassíts le, és próbáld újra!")
								end
							end
						end
					end
...
addEventHandler("onElementDataChange", getRootElement(),
	function (dataName, oldValue)
		if dataName == "char.Job" or dataName == "jobVehicle" then
			...
			if (playerJob ~= 3 and playerJob ~= 3) or not jobVehicle then
				if activeJobs[source] then
					if isElement(activeJobs[source].colshape) then
						destroyElement(activeJobs[source].colshape)
					end
