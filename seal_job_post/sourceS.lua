								...
								if getElementData(thePlayer, "char.Job") == jobID then
									...
									if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) <= 3 then
										...
										if getElementData(source, "vehicle.jobID") ~= jobID then
											outputChatBox("#d75959[SealMTA - Postás]: #ffffffEz nem munkajármű.", thePlayer, 255, 255, 255, true)
											return
										end

										if isPedInVehicle(thePlayer) then
											outputChatBox("#d75959[SealMTA - Postás]: #ffffffElőbb szállj ki a járművedből.", thePlayer, 255, 255, 255, true)
											return
										end

										if jobVehicle ~= source then
											outputChatBox("#d75959[SealMTA - Postás]: #ffffffEz nem a te munkajárműved.", thePlayer, 255, 255, 255, true)
											return
										end
										...
										if carryingMail then
											...
											if postCarInventory then
												...
												if #carryingMail == 1 then
													...
													exports.seal_chat:localAction(thePlayer, "berakott egy küldeményt az autóba.")
												else
													setPedAnimation(thePlayer, "carry", "putdwn", -1, false, false, false, false)
													exports.seal_controls:toggleControl(thePlayer, {"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, true)
													exports.seal_chat:localAction(thePlayer, "berakott néhány küldeményt az autóba.")
												end
											end
										else
	...
	function (postCarElement, packageIdentifier, packageType)
		if isElement(source) then
			...
			if postCarInventory then
				...
				exports.seal_controls:toggleControl(thePlayer, {"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, false)
			end
		end
	end
)
						...
						if colData and colData[1] == thePlayer then
							...
							if carryingMail[1] then
								...
								if carryingMail[1][2] == currentJob.packageIdentifiers[deliveryId] then
									...
									local playerPP = getElementData(thePlayer, "acc.premiumPoints") or 0
									local jobPP = math.random(25, 75)

									exports.seal_core:giveMoney(thePlayer, 50000, "postDelivered")

									setElementData(thePlayer, "acc.premiumPoints", playerPP + jobPP)
									...
									exports.seal_hud:showInfobox(thePlayer, "s", "Sikeresen leszállítottad a küldeményt és kerestél 50 000$-t és " .. jobPP .. " prémiumpontot!")
								else
									exports.seal_hud:showInfobox(thePlayer, "e", "Ez a küldemény másnak van címezve!")
								end
							end
						end
					end
