...
local maleSkins = exports.seal_binco:getSkinsByType("Férfi")
local femaleSkins = exports.seal_binco:getSkinsByType("Női")

local jobPedElement = createPed(1, 1324.0694580078, -899.43859863281, 39.578125, 0)
				...
				if getElementModel(source) == 498 then
					if getElementData(localPlayer, "char.Job") == jobID then
						if not deliveryPackages then
							outputChatBox("#d75959[SealMTA - Postás]: #ffffffMenj #4adfbfJosh#ffffff-hoz, és vedd fel a rendeléseket. (Jobbklikk az NPC-n)", 255, 255, 255, true)
							
							if not isElement(jobPedBlip) then
								local x, y, z = getElementPosition(jobPedElement)
								jobPedBlip = createBlip(x, y, z, 0, 2, 108, 179, 201)
						...
						if getDistanceBetweenPoints3D(playerX, playerY, playerZ, targetX, targetY, targetZ) <= 3 then
							if not deliveryPackages and not requestJobStart then
								...
								if isPedInVehicle(localPlayer) then
									outputChatBox("#d75959[SealMTA - Postás]: #ffffffElőbb szállj ki a járművedből.", 255, 255, 255, true)
									return
								end

								if not jobVehicle or getElementModel(jobVehicle) ~= 498 then
									outputChatBox("#d75959[SealMTA - Postás]: #ffffffElőbb kérj egy furgont a #F0E76Cmunkajármű#ffffff ikonnál.", 255, 255, 255, true)
									return
								end
...
addEventHandler("onPostmanJobStarted", localPlayer,
	function (recipientsNumber, deliveryPositions, packageType, packageIdentifier)
		...
		for i = 1, recipientsNumber do
			...
			local markerElement = createMarker(position[1], position[2], position[3] + 1.5, "arrow", 0.3, 108, 179, 201)
			local blipElement = createBlip(position[1], position[2], position[3], 0, 2, 108, 179, 201)
		...
		exports.seal_controls:toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, false)

		pdaFont = dxCreateFont("files/pda.ttf", 20, false, "antialiased")
		addEventHandler("onClientRender", getRootElement(), renderPDA)

		outputChatBox("#d75959[SealMTA - Postás]: #ffffffFelvetted a küldeményeket. Tedd be a kocsiba a szállítmányt. (Jobbklikk az autó hátuljára)", 255, 255, 255, true)
		outputChatBox("#d75959[SealMTA - Postás]: #ffffffA #4adfbfcímzetteket#ffffff a térképen és a #4adfbfPDA-n#ffffff látod.", 255, 255, 255, true)
		outputChatBox("#d75959[SealMTA - Postás]: #ffffffA PDA-n a címzettek között a nyilakkal tudsz navigálni.", 255, 255, 255, true)
		outputChatBox("#d75959[SealMTA - Postás]: #ffffffAz autód csomagtartóját a jobbklikk segitségével nyithatod meg.", 255, 255, 255, true)
	end
)
...
addEventHandler("openPostCar", getRootElement(),
	function ()
		...
		if isElement(postCarElement) and not opened then
...
addEventHandler("postDeliveredSuccessfully", getRootElement(),
	function (packageId, remainingPackages)
		...
		exports.seal_controls:toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "enter_passenger", "crouch"}, true)
...
function endTheJob(forced)
	if deliveryPackages then
		...
		if not forced then
			...
			outputChatBox("#d75959[SealMTA - Postás]: #ffffffLeszállítottad az összes küldeményt. Új kör megkezdéséhez menj #4adfbfJosh#ffffff-hoz.", 255, 255, 255, true)
		else
			if isElement(jobPedBlip) then
				destroyElement(jobPedBlip)
			end
			...
			if string.find(activeButton, "get_out:") then
				...
				if packageIdentifier then
					if postCarInventory[packageIdentifier] then
						...
						exports.seal_chat:localActionC(localPlayer, "kivett egy " .. (packageType == 1 and "levelet" or "csomagot") .. " a kocsiból.")
					else
						return
					end
				end
...
function renderInventory()
	...
	dxDrawImage(x, y + 32, sx, sy - 32, ":seal_hud/files/images/vin.png")

	-- ** Cím
	dxDrawRectangle(x, y, sx, 32, tocolor(0, 0, 0, 200))
	dxDrawText("Postakocsi", x + 4, y, x + sx, y + 32, tocolor(108, 179, 201), 1, Roboto, "left", "center")
	...
	if empty then
		...
		for k in pairs(postCarInventory) do
			...
			if activeButton == buttonName then
				dxDrawRectangle(buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], tocolor(108, 179, 201))
			else
				dxDrawRectangle(buttons[buttonName][1], buttons[buttonName][2], buttons[buttonName][3], buttons[buttonName][4], tocolor(108, 179, 201, 165))
			end
				...
				if #dataValue == 1 then
					if dataValue[1][1] == 1 then
						...
						if isElement(objectElement) then
							...
							exports.seal_boneattach:attachElementToBone(objectElement, source, 12, 0, 0.05, 0, 270, 0, 0)
						...
						if isElement(objectElement) then
							...
							exports.seal_boneattach:attachElementToBone(objectElement, source, 12, 0.2, 0.1, 0.15, 0, 90, 70)
							table.insert(carriedObjects[source], objectElement)
						end
					end
				else
					...
					if isElement(objectElement) then
						...
						exports.seal_boneattach:attachElementToBone(objectElement, source, 12, 0.25, 0.3, 0.15, -100, 0, -20)
						table.insert(carriedObjects[source], objectElement)
					end
					...
					if isElement(objectElement) then
						...
						exports.seal_boneattach:attachElementToBone(objectElement, source, 3, 0, -0.3, 0, 0, 0, 180)
						table.insert(carriedObjects[source], objectElement)
					end
				end
			else
