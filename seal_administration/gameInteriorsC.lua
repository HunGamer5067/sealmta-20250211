...
addEventHandler("onClientResourceStart", getRootElement(),
	function (startedResource)
		if getResourceName(startedResource) == "seal_hud" then
			responsiveMultipler = exports.seal_hud:getResponsiveMultipler()
		elseif getResourceName(startedResource) == "seal_interiors" then
			gameInteriors = exports.seal_interiors:getGameInteriors()
		elseif startedResource == getThisResource() then
			local seal_hud = getResourceFromName("seal_hud")

			if seal_hud and getResourceState(seal_hud) == "running" then
				responsiveMultipler = exports.seal_hud:getResponsiveMultipler()
			end

			local seal_interiors = getResourceFromName("seal_interiors")

			if seal_interiors and getResourceState(seal_interiors) == "running" then
				gameInteriors = exports.seal_interiors:getGameInteriors()
			end
		end
	end
...
function createFonts()
	...
	Rubik = dxCreateFont("files/Rubik.ttf", respc(18), false, "proof")
end
							...
							if getTickCount() - clickTick >= 1000 then
								if not isPedInVehicle(localPlayer) then
									if gameInteriors[id] then
										...
										exports.seal_hud:showInfobox("e", "Nem létező interior!")
									end
								else
									exports.seal_hud:showInfobox("e", "Előbb szállj ki a járműből!")
								end
							else
								exports.seal_hud:showInfobox("e", "Ne ilyen gyorsan!")
							end
						end
					end
				end
...
function renderThePanel()
	...
	dxDrawText("#4adfbfInsight#ffffffMTA #ffffff- Interior belsők", x + respc(7), y, x + sx, y + respc(40), tocolor(200, 200, 200, 200), 0.8, Rubik, "left", "center", false, false, false, true)
	...
	for i = 1, 15 do
		...
		if v then
			dxDrawText("#4adfbf[" .. v[1] .. "] #ffffff" .. v[2].name, x + respc(5), y2, x + sx, y2 + oneButtonHeight, tocolor(200, 200, 200, 200), 0.65, Rubik, "left", "center", false, true, false, true)
			
			local btnSizeX = dxGetTextWidth("Megtekint", 1, Rubik) + respc(2.5) + respc(20)
			local btnSizeY = dxGetFontHeight(1, Rubik) + respc(5)
			local btnName = "view_" .. v[1]

			buttons[btnName] = {x + sx - btnSizeX - respc(15), y2 + oneButtonHeight / 2 - btnSizeY / 2, btnSizeX, btnSizeY}

			drawButton(btnName, "Megtekint", buttons[btnName][1], buttons[btnName][2], buttons[btnName][3], buttons[btnName][4], {94, 193, 230}, false, Rubik, true)
		end
	end
	...
	if #interiorList > 15 then
		...
		dxDrawRectangle(x + sx - respc(5), y + respc(40) + visibleItem * contentRatio, respc(5), contentRatio * 15, tocolor(94, 193, 230, 150))
	end
	...
	if utf8.len(searchInputText) < 1 and not activeSearchInput then
		dxDrawText("Keresés...", buttons.searchInput[1] + respc(5), buttons.searchInput[2], buttons.searchInput[1] + buttons.searchInput[3] - respc(5), buttons.searchInput[2] + buttons.searchInput[4], tocolor(200, 200, 200, 200), 0.75, Rubik, "left", "center", true)
	else
		if activeSearchInput and cursorState then
			dxDrawText(searchInputText .. "|", buttons.searchInput[1] + respc(5), buttons.searchInput[2], buttons.searchInput[1] + buttons.searchInput[3] - respc(5), buttons.searchInput[2] + buttons.searchInput[4], tocolor(200, 200, 200, 200), 0.75, Rubik, "left", "center", true)
		else
			dxDrawText(searchInputText, buttons.searchInput[1] + respc(5), buttons.searchInput[2], buttons.searchInput[1] + buttons.searchInput[3] - respc(5), buttons.searchInput[2] + buttons.searchInput[4], tocolor(200, 200, 200, 200), 0.75, Rubik, "left", "center", true)
		end
	end
