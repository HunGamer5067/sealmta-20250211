...
addCommandHandler("togvá",
	function ()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			if answerState then
				outputChatBox("#4adfbf[SealMTA - #4adfbfAdminNapló#4adfbf]: #ffffffSikeresen kikapcsoltad a /vá-kat!", 255, 255, 255, true)
				answerState = false
			else
				outputChatBox("#4adfbf[SealMTA - #4adfbfAdminNapló#4adfbf]: #ffffffSikeresen bekapcsoltad a /vá-kat!", 255, 255, 255, true)
				answerState = true
			end
		end
	end)
	...
	function (message)
		if answerState then
			...
			if adminLevel >= 1 then
				outputChatBox("#4adfbf[SealMTA - #4adfbfAdminNapló#4adfbf]: #ffffff" .. message, 255, 255, 255, true)
			end
		end
	end)
...
addEvent("changeFov", true)
addEventHandler("changeFov", resourceRoot,
	function (fov)
