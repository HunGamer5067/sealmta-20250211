...
addEventHandler("placeCopier", getRootElement(),
	function (x, y, z, rz, int, dim)
		if isElement(source) and client and client == source and getElementData(source, "acc.adminLevel") >= 6 then
			...
			dbQuery(
				function (qh, sourcePlayer)
					...
					if result then
						...
						outputChatBox("#DC143C[SealMTA]: #ffffffA fénymásoló sikeresen létrehozva. #4aabd0(" .. result.copierId .. ")", sourcePlayer, 0, 0, 0, true)
					else
						outputChatBox("#DC143C[SealMTA]: #ffffffA fénymásoló létrehozása meghiúsult.", sourcePlayer, 0, 0, 0, true)
					end
				end, {source},
	...
	function (sourcePlayer, commandName, copierId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 7 then
			...
			elseif not storedCopiers[copierId] then
				outputChatBox("#DC143C[SealMTA]: #ffffffA kiválasztott fénymásoló nem létezik.", sourcePlayer, 0, 0, 0, true)
			else
				...
				outputChatBox("#4adfbf[SealMTA]: #ffffffA fénymásoló sikeresen törölve.", sourcePlayer, 0, 0, 0, true)
			end
		end
	end)
	...
	function (item, copierElement)
		if isElement(source) and client and client == source then
			if item and type(item) == "table" then
				if exports.seal_core:getMoney(source) - 10 >= 0 then
					if getCurrentWeight(source) + getItemWeight(item.itemId) <= getWeightLimit("player", source) then
						local state = false

						-- adásvételi & jegyzet
						if item.itemId == 309 or item.itemId == 72 then
						...
						-- iratok
						elseif item.itemId == 65 or item.itemId == 66 or item.itemId == 200 or item.itemId == 68 then --or item.itemId == 200 fegyver engedély
						...
						if state then
							exports.seal_core:takeMoney(source, 10, "copyNote")
							triggerClientEvent(getElementsByType("player"), "copierEffect", copierElement)
						else
							exports.seal_hud:showInfobox(source, "e", "Nincs szabad slot az inventorydban!")
						end
					else
						exports.seal_hud:showInfobox(source, "e", "Az inventoryd nem bírja el a tárgyat!")
					end
				else
					exports.seal_hud:showInfobox(source, "e", "Nincs elég pénzed a fénymásoló használatához. (10 $)")
				end
			end
		end
	end)
