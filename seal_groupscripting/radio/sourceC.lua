...
--bindKey("r", "down", "chatbox", "Rádió")

addCommandHandler("ar",
	function (commandName, ...)
		if not (...) then
			outputChatBox("#4adfbf[SealMTA - Chat]: #ffffff/" .. commandName .. " [szöveg]", 255, 255, 255, true)
		else
			if not isPedInVehicle(localPlayer) then
				outputChatBox("#d75959[SealMTA]: #ffffffNem ülsz járműben.", 255, 255, 255, true)
				return
			end
			...
			if vehType == "Plane" or vehType == "Helicopter" then
				...
				outputChatBox("#d75959[SealMTA]: #ffffffEbben a járműben nincs légi rádió.", 255, 255, 255, true)
			end
		end
	end)
	...
	function (commandName, ...)
		if not (...) then
			...
			if model == 472 or model == 595 or model == 484 or model == 430 or model == 453 or model == 454 then
				...
				outputChatBox("#d75959[SealMTA]: #ffffffEbben a járműben nincs hajó rádió.", 255, 255, 255, true)
			end
		end
	end)
...
addEventHandler("onAirRadio", getRootElement(),
	function (message, vehicles)
		if isElement(source) then
			...
			local model = getElementModel(currVeh)
			local vehType = exports.seal_vehiclenames:getCustomVehicleName(model)
...
addEventHandler("onShipRadio", getRootElement(),
	function (message, vehicles)
		if isElement(source) then
			local currVeh = getPedOccupiedVehicle(source)
			local vehType = exports.seal_vehiclenames:getCustomVehicleName(getElementModel(currVeh))
			local visibleName = getElementData(source, "visibleName"):gsub("_", " ")
