...
addEventHandler("placeStinger", getRootElement(), function (model, pos, rot, int, dim)
	if source == client and model then
		if not getElementData(source, "loggedIn") then
			return
		end

		if (exports.seal_groups:isPlayerHavePermission(client, "roadBlock") or getElementData(client, "acc.adminLevel") >= 1) and not getPedOccupiedVehicle(client) then
...
addEventHandler("syncStingerStates", getRootElement(),
	function (wheelstates)
		if isElement(source) and wheelstates then
			setTimer(setVehicleWheelStates, 1500, 1, source, unpack(wheelstates))
		end
	end)