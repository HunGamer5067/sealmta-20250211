					...
					for i = 1, #tempWheels do
						...
						if hitElement == obj then
							...
							if wheelstates[i] == 0 then
								wheelstates[i] = 1
								setTimer(setVehicleWheelStates, 1500, 1, currVeh, wheelstates[1], wheelstates[2], wheelstates[3], wheelstates[4])
...
function nearbyStingers()
	if exports.seal_groups:isPlayerHavePermission(localPlayer, "roadBlock") or getElementData(localPlayer, "acc.adminLevel") >= 1 then
		...
		if nearbyRender then
			...
			outputChatBox("#4adfbf[SealMTA - Spike]: #FFFFFFKözeli spike mód #4adfbfbekapcsolva#ffffff!", 255, 255, 255, true)
		else
			...
			outputChatBox("#4adfbf[SealMTA - Spike]: #FFFFFFKözeli spike mód #d75959kikapcsolva#ffffff!", 255, 255, 255, true)
		end
	end
end
		...
		if dat and isElement(dat[2]) and isElementStreamedIn(dat[2]) then
			...
			if dist <= 10 then
				...
				if x and y then
					...
					dxDrawText("[SPIKE] ID: " .. dat[1] .. " Lerakta: " .. dat[3], x2 + 1, y2 + 1, x2 + 200 + 1, y2 + 20 + 1, tocolor(0, 0, 0), 1, panelFont, "center", "center")
					dxDrawText("#4adfbf[SPIKE]#ffffff ID: " .. dat[1] .. " Lerakta: " .. dat[3], x2, y2, x2 + 200, y2 + 20, tocolor(200, 200, 200, 200), 1, panelFont, "center", "center", false, false, false, true)
					
					x = x + dxGetTextWidth("[SPIKE] ID: " .. dat[1] .. " Lerakta: " .. dat[3], 1, panelFont) / 2 + respc(5)
					...
					dxDrawText("Törlés", x, y - respc(10), x + respc(75), y + respc(10), tocolor(200, 200, 200, 200), 1, panelFont, "center", "center")
				end
			end
		end
	end
