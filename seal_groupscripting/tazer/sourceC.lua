		...
		if dataName == "tazerState" then
			...
			if getElementData(source, "tazerState") then
				...
				if isElement(tazerObject) then
					...
					--exports.seal_boneattach:attachElementToBone(tazerObject, source, 12, 0, 0, 0, 0, -90, 0)
		...
		if weapon == 24 and getElementData(source, "tazerState") then
			if getDistanceBetweenPoints3D(startX, startY, startZ, hitX, hitY, hitZ) <= 10 then
				...
				if isElement(hitElement) then
					...
					local sourcePlayerPos = {getElementPosition(localPlayer)}
					local targetPlayerPos = {getElementPosition(hitElement)}
